...
def main():
    ...
    _meta = _prov.provenance("mc_production.py",
                   ...
                   csf_note="csf gamma at nominal 2 mm. gamma_se_indep is the PRIMARY reported "
                            "uncertainty: the zero-covariance propagated SE (quadrature of the 2L and "
                            "3L standard errors), which assumes no correlation between the runs. gamma_se "
                            "is a launch-index-matched batch-ratio cross-check: SD(gamma_b)/sqrt(B) over "
                            "the per-batch ratios gamma_b=f3L,b/f2L,b, where batch b draws on the same "
                            ...
                            "geometries' active sets differ. Neither estimator is uniformly larger "
                            "across the calibrated configurations. "
