...
\item The partial-volume correction is beneficial at long separations. In the synthetic validation it reduces HbO$_2$ RMSE by 40--44\% at 38--40\,mm and HbR by 82--84\%, is consistently positive by $\sim$35\,mm (all 30 noise seeds; Supplementary Table~\ref{tab:multiseed}), but is harmful at 25\,mm ($-$60\% for HbO$_2$): the severe dilution places the cortical signal at or below the assumed OD-noise floor (SNR\,$<$\,1 at 25\,mm), and $\kappa_{\mathrm{PV}}$ then amplifies noise. We therefore recommend the method for SDS\,$\gtrsim$\,35\,mm (strongest at 38--40\,mm), with the exact onset set by the instrument noise.
...
\begin{table}[ht]
\centering
\caption{Monte-Carlo uncertainty and provenance for the two-layer cortical sensitivity fraction, from the single production run (\texttt{mc\_production.py}; anisotropic Henyey--Greenstein $g=0.9$, converged $L_{\max}=1200$\,mm and $z_{\max}=150$\,mm; Supplementary Table~\ref{tab:mc_conv}). $f_{\mathrm{cortex}}$ is a \emph{ratio} estimator recomputed on each of $B=16$ \emph{independent} disjoint photon batches (each $\approx$1/16 of the $2\times10^{6}$ detected photons). We report three distinct quantities: the \emph{batch SD} (spread of the individual batch estimates); the \emph{standard error} of the combined production estimate, $\mathrm{SE}=\mathrm{SD}/\sqrt{B}$; and the \emph{95\% confidence interval for the combined estimate}, the $t$ interval $f_{\mathrm{cortex}}\pm t_{0.975,15}\,\mathrm{SE}$ ($t_{0.975,15}=2.131$). The CI here is for the full $2\times10^{6}$-photon estimate; it is much narrower than the 2.5/97.5 percentile spread of the individual batch estimates (a batch-level prediction range, e.g.\ $[0.082,0.120]$ at 38\,nm/760, which describes 1/16-sample estimates and is \emph{not} a CI for the combined run). We do not tabulate $N_{\mathrm{eff}}=(\sum w)^2/\sum w^2$ as an uncertainty: it is an \emph{absorption-weight} effective count only (it does not capture pathlength variability or numerator--denominator covariance in the ratio), so the batch SE/CI, not $N_{\mathrm{eff}}$, is the operative uncertainty. DPF$\,=\langle L\rangle_w/\mathrm{SDS}$. The versioned output \texttt{fcortex\_production.json/.csv} is the single source of truth for all downstream tables and figures.}
\label{tab:mc_unc}
\begin{tabular}{@{}ccccccc@{}}
\toprule
...
\begin{table}[ht]
\centering
\caption{Noise-realization variability of the HbO$_2$ RMSE improvement, from a 30-seed repeated-noise evaluation (\texttt{multiseed\_operating\_regime} in the synthetic-validation script; five subjects per seed, independent noise draws, same converged $f_{\mathrm{cortex}}$). ``Fraction improving'' is the proportion of the 30 seeds with a positive improvement at that separation. The single-seed (seed~42) values of equation~(\ref{eq:improvement_hbo2}) are one draw from these distributions. The correction is harmful at 25\,mm, transitional at 30\,mm, and consistently beneficial ($30/30$ seeds) from 35\,mm upward; the exact crossover is specific to the assumed fixed $\sigma_{\mathrm{OD}}=10^{-3}$.}
\label{tab:multiseed}
\begin{tabular}{@{}cccc@{}}
\toprule
