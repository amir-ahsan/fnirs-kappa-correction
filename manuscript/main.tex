...
\item The partial-volume correction is reliable only at long separations. In the synthetic validation it reduces HbO$_2$ RMSE by 40--44\% at 38--40\,mm and HbR by 82--84\%, but it is harmful at 25\,mm ($-$60\% for HbO$_2$): the severe dilution places the cortical signal at or below the shot-noise floor (SNR\,$<$\,1 at 25\,mm), and $\kappa_{\mathrm{PV}}$ then amplifies noise. We therefore recommend the method only for SDS\,$\geq$\,38\,mm.
...
\begin{table}[ht]
\centering
\caption{Monte-Carlo uncertainty and provenance for the two-layer cortical sensitivity fraction, from the single production run (\texttt{mc\_production.py}; anisotropic Henyey--Greenstein $g=0.9$, converged $L_{\max}=1200$\,mm and $z_{\max}=150$\,mm; Supplementary Table~\ref{tab:mc_conv}). $f_{\mathrm{cortex}}$ is a \emph{ratio} estimator; its mean, SD, and 95\% interval are computed across $B=16$ \emph{independent} photon batches (disjoint subsets of the $2\times10^{6}$ detected photons), so the interval reflects the sampling variance of the ratio rather than a seed-level bootstrap. $N_{\mathrm{eff}}=(\sum w)^2/\sum w^2$ is an \emph{absorption-weight} effective count only---it does not capture pathlength variability or numerator--denominator covariance in the ratio, so the 16-batch interval (not $N_{\mathrm{eff}}$) is the operative uncertainty. DPF$\,=\langle L\rangle_w/\mathrm{SDS}$. The versioned output \texttt{fcortex\_production.json/.csv} is the single source of truth for all downstream tables and figures.}
\label{tab:mc_unc}
\begin{tabular}{@{}cccccc@{}}
\toprule
\textbf{SDS (mm)} & $\lambda$ (nm) & $f_{\mathrm{cortex}}$ (mean\,$\pm$\,SD) & 95\% CI (16 batches) & $N_{\mathrm{eff}}$ & DPF \\
\midrule
25 & 760 & $0.0418 \pm 0.0017$ & $[0.039,\,0.044]$ & 8714 & 6.36 \\
30 & 760 & $0.0642 \pm 0.0045$ & $[0.059,\,0.073]$ & 3792 & 6.54 \\
35 & 760 & $0.0879 \pm 0.0072$ & $[0.079,\,0.103]$ & 1661 & 6.60 \\
38 & 760 & $0.1010 \pm 0.0128$ & $[0.082,\,0.120]$ & 1115 & 6.72 \\
40 & 760 & $0.1098 \pm 0.0099$ & $[0.096,\,0.125]$ & 841 & 6.73 \\
\midrule
25 & 850 & $0.0434 \pm 0.0022$ & $[0.041,\,0.047]$ & 10744 & 5.20 \\
30 & 850 & $0.0633 \pm 0.0051$ & $[0.055,\,0.073]$ & 4760 & 5.32 \\
35 & 850 & $0.0850 \pm 0.0053$ & $[0.076,\,0.095]$ & 2146 & 5.37 \\
38 & 850 & $0.0983 \pm 0.0074$ & $[0.087,\,0.110]$ & 1344 & 5.37 \\
40 & 850 & $0.1071 \pm 0.0114$ & $[0.090,\,0.126]$ & 964 & 5.36 \\
\bottomrule
\end{tabular}
\end{table}
