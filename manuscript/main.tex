...
\usepackage{authblk}
% Plain-text expansions for math macros that appear in section titles, so hyperref
% builds valid PDF bookmark strings without "Token not allowed in a PDF string"
% warnings. (Headings still use \texorpdfstring for the math-shift/subscript tokens.)
...
\subsection{The \texorpdfstring{$\kappa$}{kappa} correction framework}
...
\end{equation}
which is a weighted average of all layers, not the true cortical concentration. The neural task-evoked response of interest originates in cortex, while superficial tissues can exhibit systemic fluctuations and task-correlated extracerebral responses; consequently the MBLL estimate is systematically attenuated by a factor proportional to $f_{\mathrm{cortex}}$ and contaminated by superficial physiology. Recovering the true cortical concentration requires inverting these effects.
...
\label{eq:correction}
\end{equation}
where $\Delta C_{\mathrm{MBLL}}$ is the na\"ive MBLL estimate from uncorrected optical density. In practice, the individual corrections (SSR, PV scaling) are applied at the OD level before MBLL inversion (see Section~\ref{sec:pipeline}). Equation~\eqref{eq:correction} is a \emph{schematic} summary of the multiplicative transport correction only: the scalar $\kappa_{\mathrm{applied}}=\kappa_{\mathrm{DPF}}\times\kappa_{\mathrm{PV}}$ summarizes the magnitude of the DPF and partial-volume factors and does \emph{not} represent SSR, which is an additive regression operation applied to the optical-density data before the wavelength-specific matrix inversion; the exact operator is the matrix formulation of Equation~\eqref{eq:matrix_correction} below. The multiplicative transport correction decomposes into two separable components targeting distinct physical mechanisms; the short-separation-regression variance-removal diagnostic $V_{\mathrm{SSR}}(\lambda)$ is computed per wavelength and reported as a diagnostic (Section~\ref{sec:kappa_ssr_diag}) rather than applied:
\begin{equation}
...
\end{equation}
where $\mathbf{y}_{\mathrm{SSR}} = [\Delta\mathrm{OD}_{\mathrm{SSR}}(\lambda_1),\,\Delta\mathrm{OD}_{\mathrm{SSR}}(\lambda_2)]^{\top}$ is the SSR-cleaned optical density, $\mathbf{E}$ is the $2\times2$ extinction matrix of equation~(\ref{eq:mbll_matrix}), and $\mathbf{P} = \mathrm{diag}\!\big(\ln(10)\,L\,\mathrm{DPF}(\lambda_i)\big)$ and $\mathbf{F} = \mathrm{diag}\!\big(f_{\mathrm{cortex}}(\lambda_i)\big)$ are wavelength-diagonal pathlength and sensitivity matrices. A single scalar mean of $1/f_{\mathrm{cortex}}$ does not in general commute with $\mathbf{E}^{-1}$, so the implementation applies $\mathbf{F}^{-1}$ and $\mathbf{P}^{-1}$ per wavelength \emph{before} inversion, exactly as in equation~(\ref{eq:matrix_correction}); the scalar $\kappa_{\mathrm{PV}}$ and $\kappa_{\mathrm{applied}}$ quoted throughout are descriptive summaries of the magnitude of these wavelength-specific corrections, not the operator actually applied. Within this matrix form the multiplicative structure follows from the linearity of the Beer--Lambert relationship: each bias source enters as a diagonal multiplicative factor at the OD level, and these compose by multiplication. The applied factors $\kappa_{\mathrm{DPF}}$ and $\kappa_{\mathrm{PV}}$ target distinct transport biases, while the per-wavelength $V_{\mathrm{SSR}}(\lambda)$ characterizes the separate SSR regression step (a diagnostic, not an applied correction); each quantity can therefore be evaluated separately. This separability is a key advantage of the framework, as it allows each applied factor to be validated separately and its uncertainty propagated in a transparent manner. We caution, however, that separability of the OD-level \emph{operations} does not imply statistical independence of their \emph{uncertainties}: DPF and $f_{\mathrm{cortex}}$ are both governed by the same tissue geometry and optical properties, so their forward-model uncertainties may covary. The present study propagates each factor's uncertainty but does not jointly propagate this potential DPF--$f_{\mathrm{cortex}}$ correlation.

\subsubsection{Processing order.}

In the implemented pipeline, all corrections are applied at the optical-density level \emph{before} MBLL inversion: (1)~SSR removes systemic physiological contamination from $\Delta\mathrm{OD}(\lambda)$ using the short-separation channel as a wavelength-matched reference, (2)~partial-volume (PV) scaling rescales the SSR-corrected $\Delta\mathrm{OD}(\lambda)$ by $\kappa_{\mathrm{PV}}$ to compensate for cortical sensitivity dilution, and (3)~MBLL inversion of the fully corrected $\Delta\mathrm{OD}$ yields quantitative cortical concentration estimates. This ordering is important: performing SSR and PV correction at the OD level preserves the per-wavelength structure of light transport, whereas applying them after MBLL inversion would conflate wavelength-dependent biases. The two \emph{applied} factors $\kappa_{\mathrm{DPF}}$ and $\kappa_{\mathrm{PV}}$ parameterise the multiplicative transport corrections applied at the OD stage, while $V_{\mathrm{SSR}}(\lambda)$ is a separate diagnostic of the preceding (additive) short-separation regression step and is not applied. Equation~\eqref{eq:correction} is a schematic summary of the multiplicative transport correction only; the exact pipeline is the matrix formulation of Equation~\eqref{eq:matrix_correction}, in which SSR enters as the regression that produces $\mathbf{y}_{\mathrm{SSR}}$ before inversion.

\subsubsection{Pathlength correction \texorpdfstring{$\kappa_{\mathrm{DPF}}$}{kappa\_DPF}.}
...
\subsubsection{Partial volume correction \texorpdfstring{$\kappa_{\mathrm{PV}}$}{kappa\_PV}.}
...
\subsubsection{SSR variance-removal diagnostic \texorpdfstring{$V_{\mathrm{SSR}}$}{V\_SSR}.}
...
\begin{table}[ht]
\centering
\caption{Summary of the applied correction-factor and SSR-diagnostic properties. The applied factors $\kappa_{\mathrm{DPF}}$ and $\kappa_{\mathrm{PV}}$ address distinct transport biases, while $V_{\mathrm{SSR}}$ is a separately estimated regression diagnostic (not a correction factor and not applied).}
...
$V_{\mathrm{SSR}}(\lambda)$ & SSR variance removal / & Per-wavelength OD & $\geq$1; this study \\
& short-channel-coupled var.\ (diagnostic) & regression $R^2$ & $\sim$1.0--2.8 \\
\bottomrule
\end{tabular}
\end{table}
...
Two forward models are used, with distinct roles. The \emph{production} forward model---the source of every \emph{principal} baseline cortical-sensitivity fraction $f_{\mathrm{cortex}}$ and CSF ratio $\gamma$ (the secondary robustness sweeps come from \texttt{mc\_robustness\_sweeps.py})---is a converged anisotropic white Monte Carlo (\texttt{mc\_production.py}; Section~\ref{sec:hankel_instab}), chosen because it remains valid in the low-scattering CSF layer where the diffusion approximation does not. The analytical \emph{comparison} model, described in this subsection, is the steady-state diffusion approximation to the radiative transfer equation\cite{r25,r26} via the semi-analytical Kienle two-layer solution\cite{r40}; we use it for methodological comparison and to expose the Hankel-quadrature convergence caveat (Section~\ref{sec:hankel_instab}), \emph{not} to generate quantitative $f_{\mathrm{cortex}}$. For the two-layer tissue geometry, the Kienle fluence solves the diffusion equation in each layer subject to boundary conditions at the extrapolated tissue surface, at the interface between layers (enforcing continuity of fluence $\Phi$ and flux $-D\nabla\Phi$), and in the deep regions, with the Hankel-domain solution numerically transformed to real space via zeroth-order Hankel transform.
...
We therefore compute $f_{\mathrm{cortex}}$ from a converged Henyey--Greenstein white Monte Carlo of the identical two-layer slab (\texttt{mc\_production.py}; $g=0.9$, $\mu_s=\mu_s'/(1-g)$, Fresnel surface, partial-pathlength scoring, $2\times10^{6}$ photons per configuration at converged $L_{\max}=1200$\,mm, $z_{\max}=150$\,mm; Supplementary Tables~\ref{tab:mc_unc}--\ref{tab:mc_conv}), whose weighted mean pathlength reproduces the expected DPF (6.4--6.7 at 760\,nm). The resulting 760\,nm fractions ($f_{\mathrm{cortex}} \approx 0.042, 0.064, 0.077, 0.088, 0.101, 0.110, 0.123$ at 25, 30, 33, 35, 38, 40, 42\,mm; the grid spans the in-vivo channel range) fall in the same low band as published atlas Monte Carlo (Section~\ref{sec:atlas_compare}); they replace the analytical-kernel values throughout. Increasing the Hankel resolution alone moves the analytical value in the right direction (at 30\,mm it falls to $\approx0.085$ by $n_s=4000$, versus the Monte-Carlo $\approx0.064$), so a singularity-subtracted or adaptive-quadrature scheme is a plausible route to a converged fast kernel; we do \emph{not}, however, claim such a scheme has been demonstrated interchangeable with the Monte Carlo here---the present package supplies no singularity-subtracted implementation or SDS-by-SDS benchmark, and any analytical route should be validated against the converged Monte Carlo before quantitative use.
...
\subsubsection{Sensitivity to \texorpdfstring{$f_{\mathrm{cortex}}$}{f\_cortex} error.}
...
The analysis proceeded in six steps: (1)~standard MBLL inversion on the raw $\Delta\mathrm{OD}(\lambda)$ without any correction, providing the biased baseline estimate against which the corrected pipeline is compared; (2)~short-separation regression (SSR) in the optical density domain: the long-channel $\Delta\mathrm{OD}(\lambda)$ is regressed against the short-channel $\Delta\mathrm{OD}(\lambda)$ at the \emph{same} wavelength $\lambda$ to remove superficial physiological contamination\cite{r15,r18,r19}; (3)~application of the wavelength-specific partial volume correction at the optical density level, dividing the SSR-cleaned $\Delta\mathrm{OD}(\lambda)$ by $f_{\mathrm{cortex}}(\lambda)$ before spectroscopic inversion; (4)~MBLL inversion on the corrected optical densities using wavelength-specific DPF values from equation~(\ref{eq:scholkmann_dpf}), $\mathrm{DPF}(760\,\mathrm{nm}) = 6.15$ and $\mathrm{DPF}(850\,\mathrm{nm}) = 5.09$ for age~25\cite{r14}, with per-wavelength normalization including the $\ln(10)$ conversion factor as in equation~(\ref{eq:mbll_matrix}), yielding the corrected $\Delta[\mathrm{HbO}_2]$ and $\Delta[\mathrm{HbR}]$ time courses; (5)~computation of the two correction factors $\kappa_{\mathrm{DPF}}$ and $\kappa_{\mathrm{PV}} = 1/f_{\mathrm{cortex}}$, together with the per-wavelength $V_{\mathrm{SSR}}(\lambda) = 1/(1-R^2_{\mathrm{SS}}(\lambda))$ diagnostic via equation~(\ref{eq:kappa_ssr}), where $R^2_{\mathrm{SS}}(\lambda)$ is the ordinary regression $R^2$ of the \emph{uncorrected} long-channel $\Delta\mathrm{OD}(\lambda)$ on the short-channel $\Delta\mathrm{OD}(\lambda)$ (the fraction of long-channel variance removed by the pipeline's actual SSR regression; here, the synthetic OLS-with-intercept fit on unfiltered OD) at each wavelength independently; the applied correction is $\kappa_{\mathrm{applied}} = \kappa_{\mathrm{DPF}} \times \kappa_{\mathrm{PV}}$ (note: since the simulation uses the true DPF for both forward model and inversion, $\kappa_{\mathrm{DPF}} = 1.000$ by design, so the corrected concentrations in table~\ref{tab:results} reflect the OD-level PV correction together with SSR denoising), while $V_{\mathrm{SSR}}(\lambda)$ is reported per wavelength as a variance-removal diagnostic of the wavelength-specific short-channel-coupled long-channel variance (ordinary regression $R^2_{\mathrm{SS}}$) and is \emph{not} applied; no cross-wavelength mean or composite $\kappa_{\mathrm{total}}$ is formed, because the per-wavelength $V_{\mathrm{SSR}}$ values differ substantially (roughly 2--2.5$\times$) and their average would be meaningless; and (6)~evaluation against ground truth using root-mean-square error:
\begin{equation}
...
\subsection{Quantitative results: \texorpdfstring{HbO$_2$}{HbO2}}

Table~\ref{tab:results} presents the correction factors and error metrics for each SDS. The partial volume correction $\kappa_{\mathrm{PV}} = 1/f_{\mathrm{cortex}}$ ranged from 9.1 (40\,mm) to 23.4 (25\,mm), confirming that partial volume dilution is by far the dominant source of MBLL bias. The SSR diagnostic (here the ordinary regression $R^2_{\mathrm{SS}}$ of the long channel on the short channel, using the same variance-removal definition as the real-data analysis, although the preprocessing and regression implementations differ) is strongly wavelength-dependent: $V_{\mathrm{SSR}}(760\,\mathrm{nm}) \approx 1.0$ while $V_{\mathrm{SSR}}(850\,\mathrm{nm}) \approx 2.4$--$2.5$ across separations, so a single cross-wavelength value is not meaningful. The simulated short channel contains no explicit cortical component; nevertheless, cortical--systemic covariance can allow SSR to remove task-related variance (in this synthetic model the regular 10\,s block period overlaps the 0.10\,Hz Mayer-wave component, so the task and superficial signals are partially correlated), so $V_{\mathrm{SSR}}$ remains a variance-removal diagnostic rather than a measure of cortical loss; only $\kappa_{\mathrm{PV}}$ is applied to the data (Section~\ref{sec:kappa_ssr_diag}). The error metrics reveal a strong separation dependence that is the central finding of this validation: the correction is beneficial at long SDS but harmful at the shortest, as detailed below.

\begin{table}[ht]
\centering
\caption{Correction factors and HbO$_2$ error metrics by source-detector separation. The corrected pipeline applies OD-level SSR followed by partial-volume scaling and MBLL inversion. In this synthetic validation only $\kappa_{\mathrm{PV}}$ is applied to the data; $V_{\mathrm{SSR}}$ (ordinary regression $R^2_{\mathrm{SS}}$, using the same variance-removal definition as the real-data analysis, though the preprocessing and regression implementations differ) is reported as a variance-removal diagnostic. The simulated short channel contains no explicit cortical component, but cortical--systemic covariance can still let SSR remove some task-related variance, so $V_{\mathrm{SSR}}$ is not a measure of cortical loss (see text). The $f_{\mathrm{cortex}}$ column is the wavelength-averaged fraction (mean of the 760 and 850\,nm values); the single-wavelength 760\,nm values used elsewhere (e.g.\ Supplementary Table~\ref{tab:snr}) are marginally smaller (0.041 at 25\,mm).}
\label{tab:results}
...
\midrule
\textbf{Overall} & & \textbf{14.00} & \textbf{1.02} & \textbf{2.45} & \textbf{2.030} & \textbf{2.044} \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Operating regime: harmful at short separations, consistently beneficial by \texorpdfstring{$\sim$35\,mm}{~35 mm}.}
\label{sec:scope}
...
\subsubsection{Computational efficiency.}
A single 3D sensitivity computation (Hankel transform plus full Jacobian volume integration) required $3.12 \pm 0.01$\,s (mean $\pm$ std over 3 trials) per channel at the standard grid resolution ($\Delta x = 1.0$\,mm, $\Delta z = 0.5$\,mm, $\Delta y = 2.0$\,mm). Extrapolating to a full fNIRS session with 40 channels and 2 wavelengths yields an estimated total processing time of approximately 250\,s. This timing is reported for the \emph{demonstration} fixed-point implementation only: while the analytical kernel is orders of magnitude faster than Monte Carlo, it does \emph{not} retain full accuracy for $f_{\mathrm{cortex}}$---its Hankel quadrature is not converged for the sensitivity ratio (Section~\ref{sec:hankel_instab})---so the speed is quoted to characterise the method's cost, not to recommend it as a production source of $f_{\mathrm{cortex}}$. A production-quality fast kernel would require a converged (e.g.\ singularity-subtracted or adaptive-quadrature) implementation benchmarked against the Monte Carlo.
...
A white Monte Carlo simulation of the three-layer geometry (the diffusion approximation is invalid in the near-transparent CSF layer) shows that the explicit CSF layer \textit{increases} the cortical sensitivity fraction relative to the two-layer model (table~\ref{tab:three_layer}), the well-known CSF light-piping effect. The amplification factor $\gamma = f_{\mathrm{cortex}}^{\mathrm{3L}}/f_{\mathrm{cortex}}^{\mathrm{2L}}$, computed within the \emph{same} production Monte Carlo (\texttt{mc\_production.py}; identical anisotropic Henyey--Greenstein $g=0.9$ transport as the two-layer model), with its \emph{standard error} propagated from the separately estimated 16-batch standard errors of $f^{\mathrm{2L}}$ and $f^{\mathrm{3L}}$ under a zero-covariance assumption (the estimates themselves share the seed and launch-index-matched cohorts, so this is a zero-covariance propagation choice, not a claim that the two are statistically independent; nominal 2\,mm CSF), is $1.84\pm0.03$ at 25\,mm, $1.66\pm0.02$ at 30\,mm, $1.57\pm0.04$ at 35\,mm, $1.44\pm0.04$ at 40\,mm and $1.37\pm0.05$ at 42\,mm (760\,nm; the corresponding 850\,nm ratios are $1.74$, $1.66$, $1.56$, $1.49$ and $1.42$, broadly similar over most of the calibrated grid, though at 25\,mm the wavelength difference---$|\Delta\gamma|\approx0.11$ versus a combined zero-covariance SE of $\approx0.04$---exceeds the combined Monte Carlo SE). As a cross-check, the two- and three-layer runs are batched by \emph{launch index} (batch id $=$ launch index mod 16, shared seed), so batch $b$ draws on the same launched-photon cohort in both geometries and the per-batch ratio $\gamma_b = f^{\mathrm{3L}}_b/f^{\mathrm{2L}}_b$ can be formed batchwise. This is a \emph{launch-index-matched} batching rather than a strict per-photon common-random-number scheme---the vectorized transport consumes the shared random stream in a different order once the two geometries' active-photon sets diverge---so we take the zero-covariance propagated standard error above as the primary reported uncertainty and retain the launch-index-matched batchwise ratio SE as a cross-check. Neither estimator dominates the other across all configurations (e.g.\ at 760\,nm/40\,mm the zero-covariance SE $\approx0.0445$ is \emph{smaller} than the batch-ratio SE $\approx0.0579$, whereas at other configurations it is larger), so the zero-covariance propagation is not uniformly conservative; a reader wanting a conservative displayed uncertainty may take the larger of the two at each configuration. The choice does not affect any scientific conclusion. This is a $\approx$37--84\% increase at 760\,nm across the full 25--42\,mm grid ($\approx$42--74\% at 850\,nm) that diminishes with separation as the two-layer cortical fraction itself grows (e.g.\ $\gamma_{760}=1.84$ at 25\,mm, $1.50$ at 38\,mm, $1.37$ at 42\,mm). These anisotropic values agree with an earlier isotropic-equivalent estimate within Monte-Carlo uncertainty, confirming that the ratio is robust to the scattering-anisotropy assumption. The low scattering of CSF lets photons traverse the layer with little attenuation and reach cortex that would otherwise be absorbed in the superficial tissue. We note a modelling boundary condition: the Monte Carlo applies Fresnel reflection/refraction \emph{only} at the external tissue--air surface; the internal scalp/CSF/cortex interfaces are treated as refractive-index matched (all layers $n=1.4$, consistent with Table~\ref{tab:optical}). The reported $\gamma$ therefore isolates the scattering/absorption (light-piping) contrast of the CSF layer and does \emph{not} include an internal $n_{\mathrm{CSF}}\approx1.33$ Fresnel mismatch; incorporating and testing that internal index step is left as a future sensitivity analysis. We apply the ratio $\gamma$ to the converged two-layer fractions of table~\ref{tab:results}: $f_{\mathrm{cortex}}^{\mathrm{3L}} = \gamma\,f_{\mathrm{cortex}}^{\mathrm{2L}} \approx 0.077$, $0.107$, $0.138$, $0.158$ and $0.168$ at 25, 30, 35, 40 and 42\,mm. The ratio depends on assumed CSF thickness (a thinner 1\,mm layer gives a smaller $\gamma\approx1.25$--$1.44$ across the full 25--42\,mm two-wavelength grid, now fully characterised with its own photon count and launch-index-matched batch uncertainty; the CSF thickness sweep is emitted by \texttt{mc\_production.py} into \texttt{fcortex\_production.json} and the CSV). Even with CSF light-piping the cortical fraction remains small ($\lesssim$\,0.17), so $\kappa_{\mathrm{PV}}$ stays large ($\gtrsim$\,6) and the partial-volume problem is not eliminated by including CSF; it is moderated by roughly the factor $\gamma$.

\begin{table}[ht]
\centering
\caption{Effect of an explicit CSF layer on cortical sensitivity, from the same production white Monte Carlo as the two-layer model (\texttt{mc\_production.py}; anisotropic Henyey--Greenstein $g=0.9$; nominal 2\,mm CSF; $\gamma$ with standard error propagated from the separately estimated 16-batch standard errors of $f^{\mathrm{2L}}$ and $f^{\mathrm{3L}}$ under a zero-covariance assumption; a launch-index-matched per-batch ratio provides a cross-check SE, neither estimator being uniformly larger, see text). The low-scattering CSF layer \textit{increases} $f_{\mathrm{cortex}}$ (light-piping) by the ratio $\gamma = f_{\mathrm{cortex}}^{\mathrm{3L}}/f_{\mathrm{cortex}}^{\mathrm{2L}}$; $f_{\mathrm{cortex}}^{\mathrm{3L}} = \gamma\,f_{\mathrm{cortex}}^{\mathrm{2L}}$ is shown using the converged two-layer fractions (table~\ref{tab:results}) and remains small ($\lesssim$\,0.17). The ratio is consistent with an isotropic-equivalent estimate within uncertainty and depends on CSF thickness.}
...
\subsection{Sensitivity to \texorpdfstring{$f_{\mathrm{cortex}}$}{f\_cortex} error}
\label{sec:fcortex_sensitivity}

Supplementary Table~\ref{tab:fcortex_sens} shows how the correction performance varies as the assumed $f_{\mathrm{cortex}}$ deviates from its true value, at the representative long separation of 38\,mm. With the exact $f_{\mathrm{cortex}}$ ($\pm$0\% error), the matched correction achieves a 69.1\% HbO$_2$ (37.8\% HbR) RMSE reduction. The framework is robust to moderate errors: HbO$_2$ improvement remains above 64\% from $-$10\% to $+$20\% perturbation and above 50\% from $-$20\% to $+$30\%, falling to 35\% only at $-$30\%. Performance peaks near the correct value and degrades on either side; underestimating $f_{\mathrm{cortex}}$ (negative errors) inflates $\kappa_{\mathrm{PV}}$ and degrades the estimate faster than overestimating it, with HbR turning marginally negative ($-$3\%) only at the $-$30\% extreme. This $\pm$30\% perturbation study should be interpreted as a \emph{moderate forward-model-error test}; realistic superficial-depth uncertainty can exceed this range substantially (a $\pm$2\,mm depth error alone moves $f_{\mathrm{cortex}}$ by roughly $-$47\% to $+$105\%, Section~\ref{sec:robustness}) and should be evaluated using the dedicated thickness-sensitivity analysis rather than read off this generic envelope.



Both chromophores are recovered best when the assumed $f_{\mathrm{cortex}}$ matches the truth, and performance degrades as the assumed value departs from it, falling off faster for underestimates, where the inflated $\kappa_{\mathrm{PV}}$ overcorrects and amplifies noise. HbR improvement is consistently smaller than HbO$_2$, reflecting the lower signal-to-noise ratio of the deoxy-hemoglobin channel, and remains positive except at the $-$30\% extreme. The broad plateau (HbO$_2$ $>$\,64\% across $-$10\% to $+$20\%) indicates the correction is forgiving of \emph{moderate} $f_{\mathrm{cortex}}$ error once a converged forward model places it in the correct band; larger, realistic superficial-depth uncertainty (which can drive $f_{\mathrm{cortex}}$ well outside $\pm$30\%) is addressed separately by the thickness-sensitivity analysis (Section~\ref{sec:robustness}).
...
$\kappa$-corrected window-mean ($\mu$M) & $+0.646$ & $-0.179$ \\
Net scaling factor & 4.49$\times$ & 5.9$\times$ \\
\midrule
$\kappa_{\mathrm{PV}}$ (mean per-channel, applied) & \multicolumn{2}{c}{6.68} \\
...
The uncorrected contralateral HbO$_2$ amplitude of ${\sim}0.12$\,$\mu$M lies in the sub-$\mu$M range typical of MBLL-derived motor-cortex finger-tapping responses reported by adult fNIRS studies\cite{r33,r34,r35}, reflecting the same partial-volume dilution that motivates this work. After nearest-short SSR denoising and the applied per-channel partial-volume correction, the group-mean corrected value of ${\sim}0.9$\,$\mu$M is consistent with the magnitude of cortical hemoglobin changes recovered after partial-volume correction in simultaneous fMRI/NIRS calibration studies\cite{r31,r32}. This demonstrates that the framework recovers physiologically plausible cortical hemodynamic amplitudes from real fNIRS measurements at long separations, subject to the demonstration-not-validation caveats above. The correction uses the CSF-augmented (three-layer, nominal 2\,mm CSF) $f_{\mathrm{cortex}}\approx0.15$ ($\kappa_{\mathrm{PV}}\approx6.7$; Table~\ref{tab:three_layer}), appropriate for the CSF layer of the real head; the \emph{no-CSF} two-layer value ($f_{\mathrm{cortex}}\approx0.10$, $\kappa_{\mathrm{PV}}\approx10.0$) would give a larger $\approx1.4\,\mu$M (an upper bound that ignores CSF light-piping). The nominal 2\,mm-CSF and the no-CSF two-layer models therefore bracket the estimate at $\approx0.9$--$1.4\,\mu$M. It is important to distinguish two independent sources of uncertainty in this number. The reported $\pm0.34\,\mu$M is the \emph{between-subject} standard deviation; it does \emph{not} include \emph{forward-model} uncertainty in $f_{\mathrm{cortex}}$. The forward-model sensitivity is best reported separately, as \emph{model-sensitivity bounds} rather than formal confidence limits: rescaling the group in-vivo amplitude with the wavelength-specific 1\,mm and 2\,mm CSF factors (Section~\ref{sec:csf_results}) gives $0.91\,\mu$M (2\,mm CSF), $1.02\,\mu$M (1\,mm CSF) and $1.37\,\mu$M (no CSF). This $\approx0.9$--$1.4\,\mu$M interval is specifically a \emph{CSF-model bracket} (2\,mm CSF vs 1\,mm CSF vs no CSF); it is \emph{not} a comprehensive forward-model uncertainty interval. In particular it does \emph{not} include uncertainty in the scalp/skull-to-cortex (superficial-layer) depth, which the robustness analysis identifies as the \emph{dominant} modeled uncertainty (a $\pm$2\,mm depth error alone moves $f_{\mathrm{cortex}}$ by roughly $-$47\% to $+$105\%; Section~\ref{sec:robustness}) and could therefore produce a larger shift than the CSF-model bracket. We did not propagate that depth uncertainty through the experimental dataset because subject-specific anatomy was unavailable for these participants---a limitation that directly motivates acquiring subject-specific MRI, scalp-skull ultrasound, or an age-matched atlas. Monte-Carlo sampling error in $f_{\mathrm{cortex}}$ ($\pm$3\% at 38\,mm; Supplementary Table~\ref{tab:mc_unc}) and cortical optical-property uncertainty are smaller contributors than either the CSF-model choice or the superficial-thickness assumption. The reported in-vivo amplitude also conditions on the wavelength-specific Scholkmann DPF values used in the inversion ($\mathrm{DPF}(760)=6.15$, $\mathrm{DPF}(850)=5.09$); subject-specific DPF uncertainty (DPF is age-, wavelength-, and anatomy-dependent) was not propagated. The corrected amplitude should therefore be read as a model-conditional estimate carrying a between-subject spread, an explicit CSF-model bracket, an unpropagated (but likely larger) superficial-thickness sensitivity, and an unpropagated DPF dependence, not as a precise measurement.
...
Atlas-based Monte Carlo (MC) light transport in realistic head templates is the reference standard for fNIRS forward modeling. Strangman, Li \& Zhang\cite{r10} report, for the Colin27 adult template, a brain (gray$+$white) sensitivity fraction rising from $\sim$6\% at 20\,mm to $\sim$22\% at 65\,mm SDS (gray-matter fraction $\sim$6\% at 20\,mm to $\sim$16\% at 45\,mm), i.e.\ $f_{\mathrm{cortex}} \approx 0.08$, $0.10$, $0.11$, $0.13$ at 25, 30, 35, 40\,mm. Our converged two-layer Monte Carlo ($f_{\mathrm{cortex}} = 0.042$, $0.064$, $0.088$, $0.110$ at 25, 30, 35, 40\,mm) falls in the same low-sensitivity regime as this published atlas Monte Carlo, but is systematically lower---by roughly 48\%, 36\%, 20\% and 15\% at 25, 30, 35 and 40\,mm respectively---particularly at the shorter separations. This is expected: the atlas uses a realistic anatomical model with somewhat different tissue-compartment definitions, so it is not an exact apples-to-apples match to the slab fraction. The comparison therefore provides \emph{qualitative external consistency} and supports the order of magnitude of the partial-volume correction ($\kappa_{\mathrm{PV}}\approx9$--$23$), rather than quantitatively validating the exact slab-derived values. The geometry-agnostic structure of the framework means any of these MC-derived $f_{\mathrm{cortex}}$ values can be substituted directly into $\kappa_{\mathrm{PV}} = 1/f_{\mathrm{cortex}}$ without modifying the rest of the pipeline.

\subsection{CSF layer effects and model mismatch}

The three-layer analysis reveals that the cerebrospinal fluid layer has a profound effect on cortical sensitivity. The low scattering coefficient of CSF ($\mu_s' \approx 0.25$\,mm$^{-1}$, compared with $\sim$1.0\,mm$^{-1}$ for scalp-skull and $\sim$0.8\,mm$^{-1}$ for cortex) creates a light-piping effect that \textit{increases} $f_{\mathrm{cortex}}$ by $\approx$37--84\% at 760\,nm across the 25--42\,mm grid relative to the two-layer model (white Monte Carlo; table~\ref{tab:three_layer}). This finding is consistent with previous reports of CSF light-piping effects\cite{r12} and has important implications for the $\kappa$ framework.

When the two-layer correction is applied (at 38\,mm) to data generated by a three-layer anatomy, the benefit is roughly halved but stays positive (HbO$_2$ $+$34.0\% vs.\ $+$69.1\% matched, HbR $+$14.7\% vs.\ $+$37.8\%; table~\ref{tab:mismatch}). This occurs because the two-layer model underestimates $f_{\mathrm{cortex}}$ ($\approx$0.10 versus $\approx$0.15 with CSF), yielding a $\kappa_{\mathrm{PV}}$ that is somewhat too large and partially overcorrects. The generic sensitivity analysis (Supplementary Table~\ref{tab:fcortex_sens}) shows the correction remains beneficial (HbO$_2$ improvement 34--71\%) for moderate $f_{\mathrm{cortex}}$ errors up to $\pm$30\%. The CSF-induced change \emph{exceeds} that $\pm$30\% range at long SDS ($\gamma_{760}\approx1.50$, i.e.\ $\approx+$50\% at 38\,mm), which is precisely why its impact was evaluated separately in this dedicated matched-versus-mismatched experiment rather than being read off the generic envelope; here the CSF mismatch degrades but does not reverse the correction. Including a CSF layer (via the Monte-Carlo ratio $\gamma$) is nonetheless recommended where anatomical information permits.

This result has two practical implications. First, when applying the $\kappa$ framework to real data, using a tissue model that includes the CSF layer is strongly recommended, even if the CSF thickness must be estimated from population atlases. Second, for applications where tissue geometry is uncertain, uncertainty should be propagated through the \emph{thickness-sensitivity curves} (Figure~\ref{fig:robustness}), which span the dominant $\pm$2\,mm superficial-depth effect, to establish \emph{sensitivity bounds} (not formal confidence bounds) on the corrected concentrations; the generic $\pm$30\% $f_{\mathrm{cortex}}$-error table (Supplementary Table~\ref{tab:fcortex_sens}) is a moderate-error test and understates realistic depth uncertainty.
...
\item \textit{If only a population-average head atlas is available} (the typical case for adult studies without per-subject MRI): use a CSF-augmented forward model with atlas-derived layer thicknesses (e.g., from the Colin27 template\cite{r10} or age-matched paediatric atlases), evaluated by Monte Carlo or the MC-calibrated $\gamma$ hybrid above rather than by a three-layer diffusion solution. Treat the resulting $\kappa_{\mathrm{PV}}$ as a point estimate and propagate two \emph{distinct} geometric uncertainties separately, because the calibrations cover different quantities: (i)~\emph{CSF-thickness} sensitivity from the 1\,mm-vs-2\,mm $\gamma$ calibration (Section~\ref{sec:csf_results})---note that a 3\,mm CSF layer lies \emph{outside} the currently calibrated 1--2\,mm CSF range and would require an additional Monte-Carlo calibration; and (ii)~total \emph{scalp/skull-to-cortex depth} uncertainty from the 8--16\,mm superficial-layer thickness-sensitivity curves of section~\ref{sec:robustness} (which vary the \emph{total} superficial depth, not the CSF sub-layer). Report the resulting \emph{sensitivity bounds} rather than confidence bounds; the depth term typically dominates.
\item \textit{If no anatomical information is available:} default to a CSF-augmented model with nominal adult thicknesses (scalp+skull $\approx$ 10\,mm, CSF $\approx$ 2\,mm)---the Monte-Carlo $f_{\mathrm{cortex}}^{\mathrm{3L}}$ or the MC-calibrated $\gamma$ hybrid---\emph{not} the bare two-layer model. The mismatch analysis above shows that applying a two-layer correction to true three-layer anatomy \emph{degrades the correction benefit} (roughly halving it at 38\,mm) though it remains positive. In an adult head a CSF-containing model is physiologically more realistic than a bare two-layer slab, so a nominal CSF-augmented model is a reasonable default when subject-specific anatomy is unavailable; the mismatch experiment tests only this one direction, however (true three-layer anatomy corrected with a two-layer model), so the result should be reported with explicit \emph{geometry sensitivity}. Because the dominant uncertainty is superficial-layer thickness---and a realistic $\pm$2\,mm depth error moves $f_{\mathrm{cortex}}$ by roughly $-$47\% to $+$105\%, well outside the generic $\pm$30\% envelope---base the reported uncertainty bounds on the thickness-sensitivity curves (Figure~\ref{fig:robustness}) rather than on the $\pm$30\% forward-model-error test.
\item \textit{In all cases:} verify post-hoc that the corrected concentrations fall within physiologically plausible bounds for the task and population. Anomalously large corrected amplitudes (e.g., $|\Delta[\mathrm{HbO}_2]| > 10$\,$\mu$M for a routine motor task) should prompt checks for model mismatch, residual motion or systemic artifact, or low SNR (an inflated $\kappa_{\mathrm{PV}}$ amplifying noise), rather than acceptance of the value.
\end{enumerate}
...
\textit{Anatomical information (the dominant input).} The correction is most sensitive to the assumed superficial-layer thickness, and the response is asymmetric: a $\pm$2\,mm error about the 12\,mm baseline changes $f_{\mathrm{cortex}}$ by approximately $-$47\% to $+$105\% and $\kappa_{\mathrm{PV}}$ by approximately $-$51\% to $+$90\% (Section~\ref{sec:robustness})---well outside the generic $\pm$30\% forward-model-error envelope. Subject-specific thickness (from structural MRI, ultrasound of scalp-skull, or an age-matched atlas) is therefore the single most valuable input and should be obtained whenever possible. When it is unavailable, corrected amplitudes should be reported with uncertainty bounds derived from the thickness-sensitivity curves (figure~\ref{fig:robustness}); cortical absorption is the next-largest contributor.
...
Several limitations should be noted. First, while the application to experimental fNIRS data (section~\ref{sec:realdata}) demonstrates that the corrected amplitudes are physiologically plausible, the absence of a concurrent ground truth measurement (e.g., fMRI BOLD signal) means we cannot compute formal error metrics on real data. The synthetic validation provides controlled error quantification, and the real-data analysis provides an ecological demonstration (a physiological-plausibility check), but a full \textit{in vivo} accuracy \emph{validation} awaits the concurrent multimodal measurements proposed in Future Directions. Additionally, real fNIRS measurements exhibit complexities including anatomical variability, motion artifacts and non-stationary physiological signals. The strong sensitivity to superficial layer thickness and optical properties identified in our robustness analysis (section~\ref{sec:robustness}) suggests that \textit{in vivo} performance will depend critically on the accuracy of the assumed tissue model. We also note that the experimental proof-of-concept rests on a single publicly available dataset (five adult participants, one finger-tapping paradigm, and a single probe geometry whose contralateral channels span 33.4--40.9\,mm, median 37.2\,mm); broader validation across tasks, ages, probe layouts, and instruments is needed before the quantitative amplitudes can be generalized.
...
Second, the two-layer model simplifies the actual multi-layer head structure. As demonstrated by our three-layer analysis (section~\ref{sec:csf_results}), the CSF layer increases cortical sensitivity by $\approx$37--84\% (760\,nm, 25--42\,mm) through light-piping; applying a two-layer correction to three-layer anatomy at long SDS roughly halves the benefit but does not reverse it ($+$34.0\% vs.\ $+$69.1\% for HbO$_2$; section~\ref{sec:mismatch_results}). Including CSF (via the Monte-Carlo ratio $\gamma$) is recommended when anatomical information is available. The generic sensitivity analysis shows HbO$_2$ correction tolerates $f_{\mathrm{cortex}}$ errors up to $\pm$30\% ($\geq$35\% improvement at 38\,mm); the CSF-induced change, however, \emph{exceeds} that $\pm$30\% band at these separations (e.g.\ $\gamma_{760}\approx1.50$, i.e.\ $\approx+$50\% at 38\,mm, and still $\approx+$37--42\% at 42\,mm), which is precisely why the CSF effect was evaluated \emph{explicitly} in the dedicated matched-versus-mismatched experiment above rather than being folded into the generic $\pm$30\% envelope.

Third, the analytical Kienle diffusion calculation is used only as a \emph{methodological comparison} and is \emph{not} the source of the production cortical-sensitivity fractions: the principal two- and three-layer $f_{\mathrm{cortex}}$ and CSF values are obtained from white Monte Carlo, precisely because the diffusion approximation is unreliable in the thin, low-scattering CSF layer---not because scattering fails to dominate absorption (it does; $\mu_s'/\mu_a\approx0.25/0.004\approx60$), but because the 1--2\,mm CSF layer is thinner than one transport mean free path ($1/\mu_s'\approx4$\,mm for $\mu_s'\approx0.25$\,mm$^{-1}$), so photons experience too few direction-randomizing events for the many-scattering assumption underlying diffusion to be satisfied. Diffusion-specific limitations---error growth near layer boundaries, particularly at the tissue--CSF interface\cite{r12,r39}---therefore apply to the analytical comparison, not to the reported quantities. The production Monte Carlo instead remains subject to its own assumptions: the two-layer slab geometry, the assumed layer optical properties, and the internal refractive-index-matched boundary treatment (Fresnel only at the tissue--air surface; Section~\ref{sec:csf_results}).

Fourth, a deeper limitation concerns the partial-volume regime itself. Because the converged $f_{\mathrm{cortex}}$ is small ($\sim$0.04--0.11), the cortical optical-density signal is intrinsically a small fraction of the measurement, and at short separations it falls below the assumed OD-noise floor (Section~\ref{sec:scope}). No multiplicative correction can recover a signal that is not present above the noise; this is a hard limit of continuous-wave, single-distance fNIRS at short SDS, not a deficiency of the correction algebra. It is the principal reason we restrict the method to long separations ($\gtrsim$35\,mm, strongest at 38--40\,mm) and rely on channel/block averaging. The exact separation at which the cortical signal crosses the noise floor scales with the measurement noise: a lower-noise instrument (smaller $\sigma_{\mathrm{OD}}$) would extend the usable range to shorter separations and a noisier one would push it longer, so the $\sim$35\,mm onset is specific to the fixed $\sigma_{\mathrm{OD}}\approx10^{-3}$ assumed here (Supplementary Table~\ref{tab:multiseed}). Relatedly, the $V_{\mathrm{SSR}}=1/(1-R^2_{\mathrm{SS}})$ term (an inverse residual-variance ratio, not an amplitude factor) is reported as a diagnostic rather than applied (Section~\ref{sec:kappa_ssr_diag}); as a diagnostic it is strongly wavelength-dependent---in simulation $V_{\mathrm{SSR}}(760\,\mathrm{nm})\approx1.0$ and $V_{\mathrm{SSR}}(850\,\mathrm{nm})\approx2.5$, and in the real data $V_{\mathrm{SSR}}(760\,\mathrm{nm})=1.78\pm0.15$ and $V_{\mathrm{SSR}}(850\,\mathrm{nm})=2.34\pm0.39$ across the five subjects---which is why it is reported per wavelength rather than as a cross-wavelength average. Task-correlated systemic responses\cite{r8,r17} can raise $R^2_{\mathrm{SS}}$ and hence this diagnostic; a high $R^2_{\mathrm{SS}}$ does not by itself establish cortical--systemic correlation, however, because shared superficial systemic variance can make it large even when the cortical and systemic components are independent. A high value therefore flags channels where SSR removed a large fraction of the long-channel variance and the corrected estimate should be treated with additional caution.
...
\item Cortical sensitivity $f_{\mathrm{cortex}}$ must be obtained from a converged forward model. We show that the analytical two-layer Kienle Jacobian, evaluated by fixed-point Hankel transform, is not quadrature-converged for this integral and overestimates $f_{\mathrm{cortex}}$ by an order of magnitude at the default $n_s = 500$ setting; a converged Monte Carlo is therefore used throughout. A singularity-subtracted or adaptive-quadrature analytical kernel is a plausible faster alternative but would first need to be demonstrated quadrature-converged and benchmarked SDS-by-SDS against the Monte Carlo, which this package does not do.
\item Superficial-layer thickness is the dominant modeled uncertainty: a $\pm$2\,mm perturbation about the 12\,mm baseline changes $f_{\mathrm{cortex}}$ by approximately $-$47\% to $+$105\% and $\kappa_{\mathrm{PV}}$ by approximately $-$51\% to $+$90\% (Monte Carlo; the response is asymmetric---12$\to$14\,mm gives $f_{\mathrm{cortex}}$ $-$47\%, $\kappa_{\mathrm{PV}}$ $+$90\%, while 12$\to$10\,mm gives $+$105\% and $-$51\%). Optical properties are less influential but not negligible: a $\pm$30\% cortical-absorption perturbation changes $\kappa_{\mathrm{PV}}$ by approximately $-$35\% to $+$40\%. This motivates subject-specific thickness estimates in particular.
...
\item The cerebrospinal fluid layer \textit{increases} $f_{\mathrm{cortex}}$ by $\approx$37--84\% at 760\,nm across the 25--42\,mm grid through light-piping (white Monte Carlo). Because this exceeds the generic $\pm$30\% sensitivity band, it was evaluated in a dedicated matched-versus-mismatched experiment: omitting CSF at long SDS roughly halves the correction benefit ($+$34.0\% vs.\ $+$69.1\% for HbO$_2$) but does not reverse it; including CSF via the Monte-Carlo ratio $\gamma$ is recommended when anatomy is known.
...
$V_{\mathrm{SSR}}(760\,\mathrm{nm})$ & $1.017 \pm 0.005$ & Per-wavelength diagnostic only (not applied) \\
$V_{\mathrm{SSR}}(850\,\mathrm{nm})$ & $2.454 \pm 0.057$ & Per-wavelength diagnostic only (not applied) \\
\bottomrule
\end{tabular}
\end{table}
