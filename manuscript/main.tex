...
\item \emph{A statistical clarification (novel).} The common SSR-restoration factor $1/(1-R^2_{\mathrm{SS}})$ attributes all SSR-removed variance to cortical loss; we show this over-restores (the relevant quantity is the cortical--systemic correlation, not the long--short channel correlation) and is better reported as a diagnostic than applied. Applying it in vivo would inflate the recovered amplitude roughly twofold.
...
The multiplicative structure follows from the linear relationship between concentration and optical density in the Beer-Lambert framework: each bias source introduces a multiplicative error in concentration space, and multiplicative errors compose by multiplication. The factors $\kappa_{\mathrm{DPF}}$, $\kappa_{\mathrm{PV}}$, and the per-wavelength diagnostic $\kappa_{\mathrm{SSR}}(\lambda)$ are designed to be independent (each targets a distinct physical mechanism) so that they can be estimated separately. This separation is a key advantage of the framework, as it allows each correction to be validated independently and its uncertainty propagated in a transparent manner.
...
Short-separation regression removes superficial contamination by regressing out the short-channel signal from the long channel at the same measurement wavelength. Critically, the regression is performed in the optical density domain: the short-channel $\Delta\mathrm{OD}(\lambda)$ is regressed against the long-channel $\Delta\mathrm{OD}(\lambda)$ at the same wavelength $\lambda$, ensuring that the correction accounts for the wavelength-dependent sensitivity of the short channel. Ideally, SSR removes only the systemic component and leaves the cortical signal intact. In practice, two distinct mechanisms can cause the SSR fit to absorb cortical variance: (a)~cortical and systemic signals may be partially correlated (for example, through task-evoked systemic responses; Kirilina et al.\cite{r17}, Tachtsidis and Scholkmann\cite{r8}); and (b)~the short channel itself, even at SDS\,$<$\,15\,mm, contains a non-negligible cortical contribution because its sensitivity profile extends beyond the superficial layer\cite{r15,r19}. Both effects produce a positive long-on-short regression coefficient that does not correspond to purely systemic contamination, so SSR inadvertently attenuates the cortical component by an amount that is captured by the regression $R^2$. The data-driven $\kappa_{\mathrm{SSR}}$ derived below is robust to both mechanisms because it operates on the regression $R^2$ itself, which directly quantifies the variance removed regardless of its physiological origin.

Because the absorption and reduced scattering coefficients differ at each wavelength, the ratio of superficial to deep optical density, and hence the SSR regression residual, is wavelength-dependent. Accordingly, $\kappa_{\mathrm{SSR}}$ is computed per wavelength at the OD level, consistent with the per-wavelength SSR and PV correction steps described in section~\ref{sec:pipeline}:
\begin{equation}
\kappa_{\mathrm{SSR}}(\lambda) = \frac{1}{1 - R^2_{\mathrm{SS}}(\lambda)}
\label{eq:kappa_ssr}
\end{equation}
where $R^2_{\mathrm{SS}}(\lambda)$ is the coefficient of determination from the OLS regression of the long-channel $\Delta\mathrm{OD}(\lambda)$ on the mean short-channel $\Delta\mathrm{OD}(\lambda)$ at the same wavelength, using bandpass-filtered signals. Specifically, $R^2_{\mathrm{SS}}(\lambda) = 1 - \mathrm{Var}(\mathrm{residual}) / \mathrm{Var}(\Delta\mathrm{OD}_{\mathrm{long}})$, where the residual is the SSR-corrected signal. This $R^2$ quantifies the fraction of long-channel variance explained by, and hence removed by, the short-separation regressor. When $R^2_{\mathrm{SS}}(\lambda) \to 0$ (short channel explains none of the long-channel variance at that wavelength), SSR removes negligible signal and $\kappa_{\mathrm{SSR}}(\lambda) \to 1$. When $R^2_{\mathrm{SS}}(\lambda) > 0$, SSR removes a proportional fraction of the total variance including some cortical signal, and $\kappa_{\mathrm{SSR}}(\lambda) > 1$ restores the attenuated amplitude. This formulation follows from the variance decomposition in ordinary least squares: if the regressor explains fraction $R^2_{\mathrm{SS}}$ of the total signal variance, the residual amplitude is reduced by a factor $(1-R^2_{\mathrm{SS}})$, giving the multiplicative correction $1/(1-R^2_{\mathrm{SS}})$. The $\kappa_{\mathrm{SSR}}(\lambda)$ values reported in the summary tables are given per wavelength (and averaged over channels); we deliberately do not form a cross-wavelength mean, because the per-wavelength values differ by nearly an order of magnitude (Tables~\ref{tab:results} and~\ref{tab:kappa_stats}) and their arithmetic average is not a physically meaningful quantity.

\paragraph{Why $\kappa_{\mathrm{SSR}}$ is a diagnostic, not an applied factor.} Equation~(\ref{eq:kappa_ssr}) treats the \emph{entire} variance removed by SSR as cortical signal that must be restored. In reality the regression removes predominantly the superficial systemic component (which is exactly what SSR is intended to discard) so $1/(1-R^2_{\mathrm{SS}})$ is an \emph{upper bound} on the restoration, not the cortical loss itself. Applying it would re-inject the systemic signal SSR removed, and when combined with the (correctly large) $\kappa_{\mathrm{PV}}=1/f_{\mathrm{cortex}}$ it double-counts the dilution: in the real-data pipeline this would scale the corrected $\Delta[\mathrm{HbO}_2]$ by a further $\sim$2.6 (the mean diagnostic), inflating it to $\sim$2.9\,$\mu$M. The cortical residual that survives SSR is already recovered, in full, by $\kappa_{\mathrm{PV}}$ alone, because $f_{\mathrm{cortex}}$ is defined on the post-SSR (cortically weighted) sensitivity. We therefore \emph{report} $\kappa_{\mathrm{SSR}}(\lambda)$ as a diagnostic of how much variance SSR removed (a proxy for residual superficial contamination and for the risk of cortical co-removal), but \emph{apply} only $\kappa_{\mathrm{DPF}}$ and $\kappa_{\mathrm{PV}}$ to the optical density. This choice is used identically in the synthetic validation and the real-data pipeline.

When cortical and systemic signals are statistically independent (as is approximately the case for externally paced cognitive or motor tasks with no systemic confound), $R^2_{\mathrm{SS}}(\lambda) \approx 0$ and $\kappa_{\mathrm{SSR}}(\lambda) \approx 1$. As a diagnostic, our synthetic simulation yields $\kappa_{\mathrm{SSR}}(760\,\mathrm{nm}) \approx 1.20$ and $\kappa_{\mathrm{SSR}}(850\,\mathrm{nm}) \approx 10.3$ (means across SDS), and the five-subject real data yields $\kappa_{\mathrm{SSR}}(760\,\mathrm{nm}) = 2.20 \pm 0.96$ and $\kappa_{\mathrm{SSR}}(850\,\mathrm{nm}) = 3.02 \pm 1.28$; this pronounced wavelength dependence is precisely why no cross-wavelength average is reported. The large synthetic $\kappa_{\mathrm{SSR}}(850\,\mathrm{nm})$ has a concrete origin in the forward model: at 850\,nm the superficial physiological oscillations dominate the long-channel variance (the systemic terms are wavelength-independent in the signal model, whereas the cortical HbO$_2$/HbR mixture at 850\,nm produces a smaller task-band optical-density change than at 760\,nm), so the mean short-channel regressor explains a much larger fraction of the long-channel variance at 850\,nm ($R^2_{\mathrm{SS}} \approx 0.90$) than at 760\,nm ($R^2_{\mathrm{SS}} \approx 0.17$). This is a genuine property of the simulated variance decomposition, not an artifact of the $R^2$ estimator, and it is exactly the wavelength-dependence the per-wavelength formulation is designed to expose. Larger values flag channels where SSR removed a substantial fraction of the long-channel variance and the cortical estimate should be treated with caution.
...
Table~\ref{tab:params} summarizes the complete parameter set. The synthetic paradigm was designed to match the BIDS-NIRS-Tapping dataset\cite{r36} used in the experimental validation (section~\ref{sec:realdata}): 5 subjects, 60 tapping blocks (5\,s task, 5\,s rest), sampling rate 7.8125\,Hz, and wavelengths 760 and 850\,nm. This parallel design enables direct quantitative comparison between synthetic and experimental results. Long-channel SDS values of 25, 30, 35, 38 and 40\,mm were tested, with 38\,mm matching the median SDS of the experimental dataset and a short-channel SDS of 10\,mm.
...
A white Monte Carlo simulation of the three-layer geometry (the diffusion approximation is invalid in the near-transparent CSF layer) shows that the explicit CSF layer \textit{increases} the cortical sensitivity fraction relative to the two-layer model (table~\ref{tab:three_layer}), the well-known CSF light-piping effect. The Monte-Carlo amplification factor $\gamma = f_{\mathrm{cortex}}^{\mathrm{3L}}/f_{\mathrm{cortex}}^{\mathrm{2L}}$ is $1.80\pm0.03$ at 25\,mm, $1.73\pm0.04$ at 30\,mm, $1.63\pm0.02$ at 35\,mm and $1.41\pm0.03$ at 40\,mm, a 40--80\% increase that diminishes with separation as the two-layer cortical fraction itself approaches unity. The low scattering of CSF lets photons traverse the layer with little attenuation and reach cortex that would otherwise be absorbed in the superficial tissue. We use the method-robust \textit{ratio} $\gamma$ from the isotropic-equivalent CSF Monte Carlo and apply it to the converged Henyey--Greenstein two-layer fractions of table~\ref{tab:results}: $f_{\mathrm{cortex}}^{\mathrm{3L}} = \gamma\,f_{\mathrm{cortex}}^{\mathrm{2L}} \approx 0.074$, $0.109$ and $0.140$ at 25, 30 and 35\,mm. Even with CSF light-piping the cortical fraction remains small ($<$\,0.15), so $\kappa_{\mathrm{PV}}$ stays large ($\gtrsim$\,7) and the partial-volume problem is not eliminated by including CSF; it is moderated by roughly the factor $\gamma$.

\begin{table}[ht]
\centering
\caption{Effect of an explicit CSF layer on cortical sensitivity, from white Monte Carlo (\texttt{mc\_csf.py}). The low-scattering CSF layer \textit{increases} $f_{\mathrm{cortex}}$ (light-piping) by the Monte-Carlo amplification factor $\gamma = f_{\mathrm{cortex}}^{\mathrm{3L}}/f_{\mathrm{cortex}}^{\mathrm{2L}}$ ($\pm$0.02--0.04). The ratio is method-robust; $f_{\mathrm{cortex}}^{\mathrm{3L}} = \gamma\,f_{\mathrm{cortex}}^{\mathrm{2L}}$ is shown using the converged Henyey--Greenstein two-layer fractions (table~\ref{tab:results}) and remains small ($<$\,0.15). The isotropic-equivalent CSF MC is used only for the ratio $\gamma$, not for absolute fractions.}
\label{tab:three_layer}
\begin{tabular}{@{}ccccc@{}}
\toprule
\textbf{SDS (mm)} & $f_{\mathrm{cortex}}^{\mathrm{2L}}$ & $\gamma$ (MC) & $f_{\mathrm{cortex}}^{\mathrm{3L}}$ & \textbf{Change (\%)} \\
\midrule
25 & 0.041 & 1.80 & 0.074 & $+$80 \\
30 & 0.063 & 1.73 & 0.109 & $+$73 \\
35 & 0.086 & 1.63 & 0.140 & $+$63 \\
\bottomrule
\end{tabular}
\end{table}
...
The BIDS-NIRS-Tapping dataset contains fNIRS recordings from five participants performing a block-design finger-tapping task. Each participant completed 60 tapping blocks (5\,s task, 5\,s rest), with data acquired at 760\,nm and 850\,nm. The probe geometry included both long-separation channels (median SDS $\approx$ 38\,mm) and short-separation channels (SDS $<$ 15\,mm), enabling per-wavelength SSR. Data were stored in SNIRF format following the BIDS-NIRS standard and loaded using MNE-Python and MNE-BIDS\cite{r37}.

We processed every subject (sub-01 through sub-05) through an identical pipeline; Subject~01 is presented in detail in figures~\ref{fig:realdata_timeseries}-\ref{fig:realdata_summary} and table~\ref{tab:realdata}, and the full group-level results are reported in section~\ref{sec:realdata_group} (figure~\ref{fig:realdata_group}, table~\ref{tab:realdata_group}). The processing steps were: (1)~conversion from raw intensity to optical density using MNE's \texttt{optical\_density} function; (2)~bandpass filtering (0.01-0.1\,Hz, fourth-order Butterworth, zero-phase); (3)~per-wavelength SSR at the OD level, regressing the mean short-channel signal independently for 760\,nm and 850\,nm; (4)~$\kappa_{\mathrm{SSR}}$ estimation from regression $R^2$ statistics using $\kappa_{\mathrm{SSR}}(\lambda) = 1/(1 - R^2_{\mathrm{SS}}(\lambda))$, reported as a diagnostic only (Section~\ref{sec:kappa_ssr_diag}); (5)~partial-volume correction of the SSR-cleaned OD, $\mathrm{OD}_{\mathrm{corrected}} = \mathrm{OD}_{\mathrm{ssr}} / f_{\mathrm{cortex}}$ (i.e.\ $\kappa_{\mathrm{PV}}$ applied, $\kappa_{\mathrm{SSR}}$ \emph{not} applied), using the CSF-augmented (three-layer) $f_{\mathrm{cortex}} = \gamma\,f_{\mathrm{cortex}}^{\mathrm{2L}}$ for the median SDS ($\approx$38\,mm), where $f_{\mathrm{cortex}}^{\mathrm{2L}}$ is the two-layer Monte-Carlo value (table~\ref{tab:results}) and $\gamma$ is the CSF light-piping ratio (Section~\ref{sec:csf_results}); for the median SDS of $38.3$\,mm the ratio is obtained by linear interpolation of the tabulated $\gamma$ (Table~\ref{tab:three_layer}), giving $\gamma(38.3\,\mathrm{mm}) \approx 1.50$ and hence $f_{\mathrm{cortex}} = 0.153$ (760\,nm) and $0.156$ (850\,nm); the CSF augmentation is appropriate here because the experimental data come from real heads, which contain a CSF layer; and (6)~MBLL inversion using wavelength-specific DPFs ($\mathrm{DPF}(760) = 6.15$, $\mathrm{DPF}(850) = 5.09$; Scholkmann and Wolf\cite{r14}) and extinction coefficients from table~\ref{tab:extinction}. The uncorrected pipeline omitted steps~(3) and (5), applying only bandpass filtering and MBLL inversion. Concentration estimates were averaged across all matched source-detector channel pairs, and block-averaged hemodynamic responses were computed relative to a 5\,s pre-stimulus baseline. Peak amplitudes were extracted as the maximum absolute value of the block-averaged response within a physiological response window (2--15\,s after task onset), which brackets the HbO$_2$ peak and the later HbR extremum and prevents a pre-stimulus or late-tail excursion from being selected; the reported values are unchanged from an unwindowed maximum for this dataset.

\subsubsection{Results.}

Figure~\ref{fig:realdata_timeseries} shows the full time series ($\sim$3000\,s) for both the uncorrected and $\kappa$-corrected concentration estimates. This separation ($\approx$38\,mm) is within the long-SDS regime where the correction is reliable (Section~\ref{sec:scope}); the CSF-augmented $f_{\mathrm{cortex}} \approx 0.15$ gives a partial-volume factor ($\kappa_{\mathrm{PV}} \approx 6.5$), which is the dominant and only applied amplification. The short channel provides SSR denoising and a $\kappa_{\mathrm{SSR}}$ diagnostic, but $\kappa_{\mathrm{SSR}}$ is not applied.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/figure4_realdata_timeseries.png}
\caption{Full time series from the BIDS-NIRS-Tapping dataset (Subject~01, SDS~$\approx$~38\,mm). \textit{Top}: $\Delta[\mathrm{HbO}_2]$. \textit{Bottom}: $\Delta[\mathrm{HbR}]$. The $\kappa$-corrected traces (solid) reflect SSR denoising and the applied partial-volume correction ($\kappa_{\mathrm{PV}}\approx 6.5$) relative to the uncorrected MBLL estimates (dashed); $\kappa_{\mathrm{SSR}}$ ($760$: 2.43, $850$: 3.30) is a reported diagnostic and is not applied. Grey bars indicate tapping blocks.}
\label{fig:realdata_timeseries}
\end{figure}

Figure~\ref{fig:realdata_hrf} presents the block-averaged hemodynamic response across all 60 tapping blocks. The uncorrected HbO$_2$ response shows the expected canonical shape (a rise during the 5\,s task period peaking at approximately $+0.26$\,$\mu$M, followed by a post-stimulus undershoot), confirming that the tapping task elicits a robust cortical hemodynamic response in this dataset. After SSR denoising and the applied partial-volume correction ($\kappa_{\mathrm{PV}} = 6.49$), the peak HbO$_2$ amplitude is $+0.95$\,$\mu$M, a net scaling factor of 3.58$\times$ relative to the uncorrected value. The HbR response shows a similar pattern: the uncorrected peak of $-0.058$\,$\mu$M becomes $-0.181$\,$\mu$M after correction (3.15$\times$). The corrected HbO$_2$ amplitude ($\sim$0.9\,$\mu$M) lies squarely in the physiological range for motor-cortex finger tapping; this is the key practical outcome of demoting $\kappa_{\mathrm{SSR}}$ to a diagnostic, applying $\kappa_{\mathrm{SSR}}$ ($\approx$2.9) on top of $\kappa_{\mathrm{PV}}$ would have scaled the peak to $\sim$2.7\,$\mu$M, double-counting the dilution (Section~\ref{sec:kappa_ssr_diag}). The net scaling derives from the SSR step (which removes superficial contamination, reducing the raw peak) followed by $\kappa_{\mathrm{PV}}$ (which restores the cortical residual); $\kappa_{\mathrm{SSR}}(760)=2.43$ and $\kappa_{\mathrm{SSR}}(850)=3.30$ are reported as diagnostics of residual superficial contamination. The standard deviation envelopes indicate that inter-block variability scales proportionally with amplitude, as expected for a multiplicative correction.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/figure5_realdata_hrf.png}
\caption{Block-averaged hemodynamic response from the BIDS-NIRS-Tapping dataset (mean $\pm$ SD across 60 tapping blocks). \textit{Left}: $\Delta[\mathrm{HbO}_2]$. \textit{Right}: $\Delta[\mathrm{HbR}]$. The $\kappa$-corrected response (solid) reflects SSR denoising and the applied partial-volume correction ($\kappa_{\mathrm{PV}} = 6.49$; $\kappa_{\mathrm{SSR}}$ diagnostic only), producing 3.58$\times$ amplification of HbO$_2$ (from $\sim$0.26 to $\sim$0.95\,$\mu$M, within the physiological range). Grey shading indicates the 5\,s task period.}
\label{fig:realdata_hrf}
\end{figure}

The quantitative results are summarized in figure~\ref{fig:realdata_summary} and table~\ref{tab:realdata}. The net scaling factors (3.58$\times$ for HbO$_2$, 3.15$\times$ for HbR) recover physiological cortical amplitudes from the partial-volume-diluted measurements. The recovery is driven by the partial-volume factor $\kappa_{\mathrm{PV}} = 6.49$ (the only applied amplification), acting on the SSR-denoised optical density; the SSR step first removes superficial contamination, reducing the raw peak, after which $\kappa_{\mathrm{PV}}$ restores the diluted cortical residual. Because the real data have no ground truth, we explicitly separate these two contributions to the net HbO$_2$ scaling (Table~\ref{tab:realdata}): SSR alone \emph{reduces} the raw peak by a factor of $0.56$ for this subject ($0.71 \pm 0.13$ across the five subjects), and $\kappa_{\mathrm{PV}}$ alone \emph{amplifies} it by $6.41$ ($6.43$ across subjects, i.e.\ essentially $\kappa_{\mathrm{PV}}$ acting on the near-unchanged peak); their product reproduces the $3.58\times$ net scaling. The corrected HbO$_2$ amplitude ($+0.95\,\mu$M) is consistent with the magnitude of motor-cortex finger-tapping responses recovered after partial-volume correction in simultaneous fMRI/NIRS calibration studies\cite{r31,r32}. The net scaling ($3.58\times$) is smaller than $\kappa_{\mathrm{PV}}$ itself because SSR removes part of the raw long-channel peak before amplification. The $\kappa_{\mathrm{SSR}}(\lambda)$ values ($760: 2.43$, $850: 3.30$) are reported as diagnostics of residual superficial contamination but are deliberately not applied, which keeps the corrected amplitude physiological (Section~\ref{sec:kappa_ssr_diag}).

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/figure6_realdata_summary.png}
\caption{Summary of $\kappa$-correction applied to real fNIRS data (Subject~01, SDS~$\approx$~38\,mm). \textit{Left}: block-averaged peak concentrations for HbO$_2$ and HbR. \textit{Right}: correction parameters and numerical results.}
\label{fig:realdata_summary}
\end{figure}

\begin{table}[ht]
\centering
\caption{Real fNIRS data results from the BIDS-NIRS-Tapping dataset (Subject~01). Block-averaged peak amplitudes from 60 tapping blocks at SDS~$\approx$~38\,mm after SSR denoising and the applied partial-volume correction ($\kappa_{\mathrm{PV}}$); $\kappa_{\mathrm{SSR}}$ is a reported diagnostic and is not applied.}
\label{tab:realdata}
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Metric} & \textbf{HbO$_2$} & \textbf{HbR} \\
\midrule
Uncorrected peak ($\mu$M) & $+0.265$ & $-0.058$ \\
SSR-only peak (no PV, $\mu$M) & $+0.148$ & --- \\
SSR $+$ PV corrected peak ($\mu$M) & $+0.946$ & $-0.181$ \\
SSR-only scaling & 0.56$\times$ & --- \\
PV-only scaling (on SSR-cleaned OD) & 6.41$\times$ & --- \\
Net scaling factor & 3.58$\times$ & 3.15$\times$ \\
\midrule
$f_{\mathrm{cortex}}(760\,\mathrm{nm})$ (3-layer, CSF) & \multicolumn{2}{c}{0.153} \\
$f_{\mathrm{cortex}}(850\,\mathrm{nm})$ (3-layer, CSF) & \multicolumn{2}{c}{0.156} \\
$\kappa_{\mathrm{PV}}$ (mean, applied) & \multicolumn{2}{c}{6.494} \\
$\kappa_{\mathrm{SSR}}(760\,\mathrm{nm})$ (diagnostic) & \multicolumn{2}{c}{2.433} \\
$\kappa_{\mathrm{SSR}}(850\,\mathrm{nm})$ (diagnostic) & \multicolumn{2}{c}{3.303} \\
\midrule
Wavelengths & \multicolumn{2}{c}{760, 850\,nm} \\
Median SDS & \multicolumn{2}{c}{38.3\,mm} \\
Sampling rate & \multicolumn{2}{c}{7.8\,Hz} \\
Tapping blocks & \multicolumn{2}{c}{60} \\
\bottomrule
\end{tabular}
\end{table}
...
To assess generalisation beyond the single-subject demonstration, the same pipeline was applied to every participant in the BIDS-NIRS-Tapping dataset (sub-01 through sub-05). Per-subject metrics are reported in table~\ref{tab:realdata_group} and figure~\ref{fig:realdata_group} shows the block-averaged hemodynamic responses overlaid for all five subjects together with the group mean. All five subjects share the same probe geometry (median long-channel SDS $= 38.3$\,mm, 60 tapping blocks, 7.8\,Hz sampling), so the applied $\kappa_{\mathrm{PV}}$ is identical across subjects ($6.494$); the $\kappa_{\mathrm{SSR}}$ diagnostic, by contrast, is data-driven, strongly wavelength-dependent, and varies between subjects ($\kappa_{\mathrm{SSR}}(760\,\mathrm{nm}) = 2.20 \pm 0.96$ and $\kappa_{\mathrm{SSR}}(850\,\mathrm{nm}) = 3.02 \pm 1.28$ across the five subjects), reflecting the per-subject correlation between long- and short-channel signals in the hemodynamic band (i.e.\ how much superficial variance SSR removed). Because the two wavelengths differ substantially, $\kappa_{\mathrm{SSR}}$ is reported per wavelength rather than as a cross-wavelength mean.

\begin{table}[ht]
\centering
\caption{Per-subject and group-level $\kappa$-correction results for the full BIDS-NIRS-Tapping dataset (N$=$5; SDS $\approx$ 38\,mm; 60 tapping blocks per subject). Only $\kappa_{\mathrm{PV}}$ is applied; $\kappa_{\mathrm{SSR}}$ is reported per wavelength (760 and 850\,nm) as a diagnostic, with no cross-wavelength mean or composite $\kappa_{\mathrm{total}}$ formed (the two wavelengths differ substantially). ``Mean scale'' is the average of the HbO$_2$ and HbR net scaling factors. Subject~01 is the same case detailed in table~\ref{tab:realdata}.}
...
\midrule
sub-01 & 6.494 & 2.433 & 3.303 & 0.265 & 0.946 & 0.058 & 0.181 & 3.36$\times$ \\
sub-02 & 6.494 & 1.905 & 2.340 & 0.261 & 1.092 & 0.055 & 0.340 & 5.17$\times$ \\
sub-03 & 6.494 & 1.862 & 2.851 & 0.231 & 0.994 & 0.033 & 0.191 & 5.06$\times$ \\
sub-04 & 6.494 & 3.684 & 5.010 & 0.141 & 0.792 & 0.109 & 0.516 & 5.18$\times$ \\
sub-05 & 6.494 & 1.104 & 1.583 & 0.352 & 1.842 & 0.148 & 0.899 & 5.66$\times$ \\
\midrule
Mean & 6.494 & 2.197 & 3.017 & 0.250 & 1.133 & 0.080 & 0.425 & 4.89$\times$ \\
SD & 0.000 & 0.956 & 1.284 & 0.076 & 0.411 & 0.047 & 0.298 & 0.88$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Group-level block-averaged hemodynamic response across all five BIDS-NIRS-Tapping subjects. Thin lines show per-subject means (light = uncorrected, dark = $\kappa$-corrected); bold black lines show the group mean ($\pm$ standard error implied by the across-subject spread). The partial-volume correction ($\kappa_{\mathrm{PV}}=6.49$, applied) systematically amplifies both HbO$_2$ and HbR responses relative to the uncorrected MBLL estimate in every subject, with a group-mean amplification of $4.6\times$ (HbO$_2$) and $5.2\times$ (HbR) and a corrected HbO$_2$ amplitude of $1.13\pm0.41\,\mu$M, within the physiological range. Inter-subject variability of the net scaling reflects the per-subject SSR denoising (the $\kappa_{\mathrm{SSR}}$ diagnostic); $\kappa_{\mathrm{PV}}$ is identical across subjects because they share the same probe geometry.}
\label{fig:realdata_group}
\end{figure}

The group-mean uncorrected HbO$_2$ peak ($0.250 \pm 0.076$\,$\mu$M) and corrected peak ($1.133 \pm 0.411$\,$\mu$M) yield a group-mean scaling of $4.59 \pm 0.83\times$ for HbO$_2$ and $5.18 \pm 1.27\times$ for HbR. Critically, the corrected amplitudes (per-subject HbO$_2$: 0.9, 1.1, 1.0, 0.8, 1.8\,$\mu$M) all fall within the physiological range for motor-cortex finger tapping, whereas applying the $\kappa_{\mathrm{SSR}}$ diagnostic as a correction in addition to $\kappa_{\mathrm{PV}}$ would have inflated them to $\sim$2--4\,$\mu$M. Subject~01, used in the detailed analysis above, has a corrected HbO$_2$ peak of $0.95$\,$\mu$M, near the group lower-middle; sub-05 reaches $1.84$\,$\mu$M, partly because its small $\kappa_{\mathrm{SSR}}$ ($R^2_{\mathrm{SS}} \approx 0.26$) means SSR removed little of the raw peak before $\kappa_{\mathrm{PV}}$ amplification. Every subject shows positive amplification of physiologically plausible magnitude, consistent with the framework's premise that MBLL systematically underestimates cortical concentrations under partial-volume dilution at this long separation. With only five subjects and no concurrent ground truth, these group statistics are reported as descriptive summaries (mean\,$\pm$\,SD across subjects); we deliberately make no inferential (hypothesis-testing) claim, as the purpose of this analysis is to demonstrate physiological plausibility rather than to establish statistical significance. We stress that landing in the physiological range is a weak test on its own---many scaling factors would also produce plausible amplitudes---so the load-bearing real-data result is not that ``the number looks right'' but the \emph{checkable, methodological} contrast between reporting and applying $\kappa_{\mathrm{SSR}}$: the same pipeline that yields $\sim$1.1\,$\mu$M when $\kappa_{\mathrm{SSR}}$ is treated as a diagnostic yields $\sim$2.7\,$\mu$M (above the physiological range) when $\kappa_{\mathrm{SSR}}$ is applied, a direct and reproducible consequence of the statistical clarification in Section~\ref{sec:kappa_ssr_diag}. The diagnostic is also actionable per subject: subject~04's elevated $\kappa_{\mathrm{SSR}}$ ($760$: 3.68, $850$: 5.01) flags it as the case where SSR removed the largest fraction of long-channel variance, so its corrected amplitude ($0.79\,\mu$M) warrants more caution than subjects whose $\kappa_{\mathrm{SSR}}$ is near unity.

Importantly, these results illustrate the SDS-dependent operating regime of the correction framework, which the converged $f_{\mathrm{cortex}}$ inverts relative to a naive reading. Partial-volume dilution is severe at \emph{all} separations ($f_{\mathrm{cortex}} = 0.04$ to $0.11$, $\kappa_{\mathrm{PV}} = 9$ to $23$); $\kappa_{\mathrm{PV}}$ is the dominant correction throughout. What changes with separation is not the size of the dilution but whether the cortical signal is recoverable: at short SDS ($\leq$30\,mm) the diluted cortical optical density sits at or below the measurement-noise floor (Section~\ref{sec:scope}), so the large $\kappa_{\mathrm{PV}}$ amplifies noise and the correction is unreliable; at long SDS ($\geq$38\,mm) the cortical SNR is adequate and $\kappa_{\mathrm{PV}}$ recovers physiological amplitudes, as observed here. The real data sit at 38.3\,mm, in the reliable regime. $\kappa_{\mathrm{SSR}}(\lambda)$ is reported as a diagnostic of residual superficial contamination but is not applied, because the cortical residual surviving SSR is recovered in full by $\kappa_{\mathrm{PV}}=1/f_{\mathrm{cortex}}$; applying $\kappa_{\mathrm{SSR}}$ as well would double-count the dilution (Section~\ref{sec:kappa_ssr_diag}).

The uncorrected HbO$_2$ amplitude of ${\sim}0.24$\,$\mu$M lies in the sub-$\mu$M range typical of MBLL-derived motor-cortex finger-tapping responses reported by adult fNIRS studies\cite{r33,r34,r35}, reflecting the same partial-volume dilution that motivates this work. After SSR denoising and the applied partial-volume correction, the group-mean corrected value of ${\sim}1.1$\,$\mu$M is consistent with the magnitude of cortical hemoglobin changes recovered after partial-volume correction in simultaneous fMRI/NIRS calibration studies\cite{r31,r32}. This demonstrates that the framework recovers quantitative, physiologically plausible cortical hemodynamic amplitudes from real fNIRS measurements at long source-detector separations. This correction uses the CSF-augmented (three-layer) $f_{\mathrm{cortex}}\approx0.15$ ($\kappa_{\mathrm{PV}}\approx6.5$; Table~\ref{tab:three_layer}), appropriate for the CSF layer of the real head; the two-layer value ($f_{\mathrm{cortex}}\approx0.10$, $\kappa_{\mathrm{PV}}\approx9.7$) would give a larger $\approx1.7\,\mu$M (an upper bound that ignores CSF light-piping) so the physiological estimate is bracketed as $\approx1.1$--$1.7\,\mu$M.
...
In the multi-subject synthetic validation, the partial-volume correction reduces HbO$_2$ RMSE by 44--45\% at 38--40\,mm (and HbR by 82--89\%), while at 25\,mm it is harmful ($-$55\% for HbO$_2$). A separate, and consequential, methodological point concerns the SSR-attenuation term: $\kappa_{\mathrm{SSR}}=1/(1-R^2_{\mathrm{SS}})$ over-attributes the SSR-removed variance to cortical loss, so we report it as a diagnostic and apply only $\kappa_{\mathrm{PV}}$; this keeps the real-data corrected amplitude physiological ($\sim$1.1\,$\mu$M) instead of the $\sim$2.7\,$\mu$M that applying both factors produces.
...
The real-data analysis is a proof-of-concept demonstration (one public dataset, five subjects, no concurrent ground truth): at 38.3\,mm, the applied $\kappa_{\mathrm{PV}}\approx6.5$ (CSF-augmented three-layer $f_{\mathrm{cortex}}\approx0.15$) raises the group-mean cortical HbO$_2$ from $0.25$ to $1.13\,\mu$M ($4.6\times$), a physiologically plausible amplitude. This rests on two methodological choices established above: using a converged ($f_{\mathrm{cortex}}\approx0.10$--$0.15$) rather than an under-converged ($f_{\mathrm{cortex}}\approx0.78$) sensitivity fraction, and reporting $\kappa_{\mathrm{SSR}}$ as a diagnostic rather than applying it. With the former wrong choice, $\kappa_{\mathrm{PV}}$ would have been a near-unity correction with negligible effect; with the latter, the corrected amplitude would have been inflated to $\sim$2.7\,$\mu$M by double-counting the dilution. The pipeline runs on standard BIDS-formatted data with open-source tools (MNE-Python, MNE-BIDS) in seconds on a laptop, with $f_{\mathrm{cortex}}$ taken from a precomputed Monte-Carlo table for the probe geometry.
...
\item The applied partial-volume correction ($\kappa_{\mathrm{PV}} = 1/f_{\mathrm{cortex}}$) has a mean of $13.77 \pm 5.43$ over 25--40\,mm ($\approx$9--10 in the recommended long-SDS regime). The $\kappa_{\mathrm{SSR}}(\lambda) = 1/(1-R^2_{\mathrm{SS}})$ term is reported per wavelength as a diagnostic (simulation: $760$\,nm $\approx$1.2, $850$\,nm $\approx$10.3; real data: $760$\,nm $2.20\pm0.96$, $850$\,nm $3.02\pm1.28$; no cross-wavelength mean is formed) but \emph{not} applied, since the cortical residual surviving SSR is recovered in full by $\kappa_{\mathrm{PV}}$ and applying both double-counts the dilution.
...
\item Application to all five BIDS-NIRS-Tapping participants at 38.3\,mm recovers a group-mean cortical $\Delta[\mathrm{HbO}_2]$ of $1.13\pm0.41\,\mu$M ($4.6\times$ the uncorrected estimate), within the physiological range for finger tapping. This is a proof-of-concept on one public dataset (no concurrent ground truth) and a direct consequence of using the converged, CSF-augmented $f_{\mathrm{cortex}}$ and the $\kappa_{\mathrm{SSR}}$-as-diagnostic choice.
...
The experimental fNIRS recordings used in section~\ref{sec:realdata} are publicly available from the BIDS-NIRS-Tapping repository (Luke et al., 2021; \mbox{DOI: 10.5281/zenodo.5529797})\cite{r36}. The complete Python implementation accompanying this manuscript (including the production analysis pipeline (\texttt{fnirs\_\allowbreak kappa\_\allowbreak synthetic\_\allowbreak validation.py}), the per-subject real-data script (\texttt{fnirs\_\allowbreak kappa\_\allowbreak realdata\_\allowbreak analysis.py}), the multi-subject group-analysis script (\texttt{fnirs\_\allowbreak kappa\_\allowbreak group\_\allowbreak analysis.py}), the in-vivo demonstration script, the two-layer and CSF Monte-Carlo forward models, and an extensively commented pedagogical Jupyter notebook) is provided as a self-contained reproducibility package deposited in a public repository with a citable DOI. All synthetic data are reproducible by re-running the supplied code with the published random seeds (synthetic \texttt{seed}~$=$~42; Monte-Carlo \texttt{seed}~$=$~1); the SNR table (Supplementary Table~\ref{tab:snr}) and the SSR/PV scaling decomposition (Table~\ref{tab:realdata}) are emitted directly by the supplied scripts.
