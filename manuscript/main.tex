...
A particularly promising direction arising from the three-layer analysis is the development of an empirical CSF correction factor that preserves the computational efficiency of the two-layer Kienle solution while incorporating the CSF light-guiding effect. Our anisotropic Monte Carlo shows that the ratio $\gamma = f_{\mathrm{cortex}}^{\mathrm{3L}} / f_{\mathrm{cortex}}^{\mathrm{2L}}$ varies systematically with SDS: $1.84$ at 25\,mm, $1.66$ at 30\,mm, $1.57$ at 35\,mm, $1.44$ at 40\,mm and $1.37$ at 42\,mm (2\,mm CSF; table~\ref{tab:three_layer}). In practice, one could compute $f_{\mathrm{cortex}}$ rapidly via the two-layer Hankel transform ($\sim$0.1\,s per channel) and then multiply by an SDS-dependent correction factor $\gamma(d_{\mathrm{SDS}}, d_{\mathrm{CSF}})$ derived from a precomputed lookup table parameterized by source-detector separation and assumed CSF thickness. The adjusted sensitivity $f_{\mathrm{cortex}}^{\mathrm{adj}} = \gamma \times f_{\mathrm{cortex}}^{\mathrm{2L}}$ would then enter the $\kappa_{\mathrm{PV}}$ calculation. This hybrid approach retains the speed advantage of the analytical two-layer solution while accounting for the dominant source of model mismatch identified in this study, and---because the CSF correction is supplied by Monte Carlo rather than by a diffusion solution inside the low-scattering CSF---it avoids applying diffusion theory where it is invalid. We therefore recommend it as the \emph{practical default} when a full Monte-Carlo forward model is not run, used within the range over which $\gamma$ is calibrated here (SDS 25--42\,mm, CSF thickness 1--2\,mm, 760 and 850\,nm, adult two-layer slab anatomy); it is calibrated against the production Monte Carlo (\texttt{mc\_production.py}, which emits the $\gamma$ values and a 1/2\,mm CSF-thickness sweep). Extending the lookup table across CSF thicknesses, wavelengths, and probe geometries---and validating it against atlas Monte Carlo---is a natural next step before use outside this range.
