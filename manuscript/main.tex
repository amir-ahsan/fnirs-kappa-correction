...
30 & 0.064 & $1.66\pm0.03$ & 0.107 & $+$66 \\
35 & 0.088 & $1.57\pm0.03$ & 0.138 & $+$57 \\
...
\caption{Per-subject amplification for the five BIDS-NIRS-Tapping subjects (contralateral long channels, SDS~$\approx$~38\,mm). Each point is one subject's uncorrected versus $\kappa$-corrected window-mean $\Delta[\mathrm{HbO}_2]$; the dashed line is the mean per-channel $\kappa_{\mathrm{PV}}\approx6.8$ (only applied amplification) and the dotted line is the identity. Per-subject net scaling varies (4--14$\times$) because the intervening SSR step removes a subject-specific fraction of the raw peak before $\kappa_{\mathrm{PV}}$ amplification.}
...
\begin{table}[ht]
\centering
\caption{Monte-Carlo uncertainty and provenance for the two-layer cortical sensitivity fraction, from the single production run (\texttt{mc\_production.py}; anisotropic Henyey--Greenstein $g=0.9$, converged $L_{\max}=1200$\,mm and $z_{\max}=150$\,mm; Supplementary Table~\ref{tab:mc_conv}). $f_{\mathrm{cortex}}$ is a \emph{ratio} estimator recomputed on each of $B=16$ \emph{independent} disjoint photon batches (each $\approx$1/16 of the photons \emph{detected within that SDS annulus}, from the $2\times10^{6}$ photons \emph{launched} per configuration; only a subset is detected in any one annulus). We report three distinct quantities: the \emph{batch SD} (spread of the individual batch estimates); the \emph{standard error} of the combined production estimate, $\mathrm{SE}=\mathrm{SD}/\sqrt{B}$; and the \emph{95\% confidence interval for the combined estimate}, the $t$ interval $f_{\mathrm{cortex}}\pm t_{0.975,15}\,\mathrm{SE}$ ($t_{0.975,15}=2.131$). The CI here is for the full $2\times10^{6}$-photon estimate; it is much narrower than the 2.5/97.5 percentile spread of the individual batch estimates (a batch-level prediction range, e.g.\ $[0.082,0.120]$ at 38\,mm, 760\,nm, which describes 1/16-annulus-sample estimates and is \emph{not} a CI for the combined run). We do not tabulate $N_{\mathrm{eff}}=(\sum w)^2/\sum w^2$ as an uncertainty: it is an \emph{absorption-weight} effective count only (it does not capture pathlength variability or numerator--denominator covariance in the ratio), so the batch SE/CI, not $N_{\mathrm{eff}}$, is the operative uncertainty. DPF$\,=\langle L\rangle_w/\mathrm{SDS}$. The versioned output \texttt{fcortex\_production.json/.csv} is the single source of truth for all downstream tables and figures.}
\label{tab:mc_unc}
\begin{tabular}{@{}ccccccc@{}}
\toprule
\textbf{SDS (mm)} & $\lambda$ (nm) & $f_{\mathrm{cortex}}$ & batch SD & SE & 95\% CI ($t$) & DPF \\
\midrule
25 & 760 & $0.0418$ & $0.0017$ & $0.0004$ & $[0.041,\,0.043]$ & 6.36 \\
30 & 760 & $0.0642$ & $0.0045$ & $0.0011$ & $[0.062,\,0.067]$ & 6.54 \\
35 & 760 & $0.0879$ & $0.0072$ & $0.0018$ & $[0.084,\,0.092]$ & 6.60 \\
38 & 760 & $0.1010$ & $0.0128$ & $0.0032$ & $[0.094,\,0.108]$ & 6.72 \\
40 & 760 & $0.1098$ & $0.0099$ & $0.0025$ & $[0.105,\,0.115]$ & 6.73 \\
\midrule
25 & 850 & $0.0434$ & $0.0022$ & $0.0006$ & $[0.042,\,0.045]$ & 5.20 \\
30 & 850 & $0.0633$ & $0.0051$ & $0.0013$ & $[0.061,\,0.066]$ & 5.32 \\
35 & 850 & $0.0850$ & $0.0053$ & $0.0013$ & $[0.082,\,0.088]$ & 5.37 \\
38 & 850 & $0.0983$ & $0.0074$ & $0.0019$ & $[0.094,\,0.102]$ & 5.37 \\
40 & 850 & $0.1071$ & $0.0114$ & $0.0028$ & $[0.101,\,0.113]$ & 5.36 \\
\bottomrule
\end{tabular}
\end{table}
