...
\fancyhf{}
\rhead{\thepage}
\lhead{fNIRS $\kappa$-Correction Guide}
...
\begin{abstract}
This pedagogical guide introduces undergraduate physics students to functional near-infrared spectroscopy (fNIRS) and the $\kappa$-correction framework for recovering quantitative hemoglobin concentration measurements. The guide provides complete derivations, physical intuition, worked examples, and step-by-step implementations. We explain why the Modified Beer-Lambert Law (MBLL) systematically underestimates cortical hemoglobin changes by up to $10\times$ in layered tissues, and how three multiplicative correction factors ($\kappa_{\mathrm{DPF}}$, $\kappa_{\mathrm{PV}}$, and $\kappa_{\mathrm{SSR}}$) compensate for differential pathlength error, partial volume dilution, and residual superficial contamination. The framework is grounded in the diffusion approximation to light transport and the Kienle et al.~(1998) semi-analytical solution for two-layer tissue geometry.
\end{abstract}
...
However, SSR has a limitation: if cortical and superficial signals happen to be partially correlated (e.g., task-evoked systemic responses), SSR may inadvertently remove some cortical signal along with the contamination. The $\kappa_{\text{SSR}}$ factor corrects for this inadvertent attenuation.
...
\subsection{Step 2: Two Absorbers (HbO$_2$ and HbR)}
...
\newpage
\part{The $\kappa$ Correction Framework}
...
\item $\kappa_{\text{SSR}}(\lambda) = 1/(1 - R^2_{\text{SS}}(\lambda))$: computed \emph{per wavelength} and reported as a \emph{diagnostic} of residual superficial contamination; it is \emph{not} applied to the data, because applying it on top of $\kappa_{\text{PV}}$ would double-count the dilution (see the $\kappa_{\text{SSR}}$ box later in this guide)
\end{itemize}
...
\section{Derivation of Each $\kappa$ Factor}

\subsection{Factor 1: $\kappa_{\text{DPF}}$, Wavelength-Dependent Path Length}
...
\subsection{Factor 2: $\kappa_{\text{PV}} = 1/f_{\text{cortex}}$, Partial Volume Correction}
...
\subsection{Factor 3: $\kappa_{\text{SSR}}(\lambda)$, Correction for SSR Signal Loss}

Short-separation regression removes superficial contamination, but it does so by subtracting the short-channel signal from the long-channel signal:
...
where $\beta(\lambda)$ is the regression coefficient (or slope) minimizing the residual power.

If the short-channel signal contains some cortical contribution (though small), the regression will inadvertently remove some true cortical signal. The amount of cortical signal removed depends on the correlation between long and short channels in the cortical component.

Define $R^2_{\mathrm{SS}}(\lambda)$ as the fraction of variance in the \emph{uncorrected} long-channel $\Delta\mathrm{OD}$ that is explained by regressing it on the short-channel $\Delta\mathrm{OD}$. Equivalently, $R^2_{\mathrm{SS}}$ is the coefficient of determination of the SSR fit itself, \emph{not} of a post-hoc regression on the SSR residual. (By construction of ordinary least squares, the SSR residual is orthogonal to the short-channel regressor, so regressing the residual on the short channel would always return $R^2 \approx 0$ and therefore $\kappa_{\mathrm{SSR}} \approx 1$, i.e., no correction, an incorrect result.) If $R^2_{\mathrm{SS}} = 0$, the short and long channels are uncorrelated (ideal case). If $R^2_{\mathrm{SS}} = 1$, they are perfectly correlated.

The attenuation factor due to SSR is:

\begin{equation}
A_{\mathrm{SSR}} = 1 - R^2_{\mathrm{SS}}(\lambda)
\end{equation}

and the correction is:

\begin{equation}
\kappa_{\mathrm{SSR}}(\lambda) = \frac{1}{1 - R^2_{\mathrm{SS}}(\lambda)} = \frac{1}{A_{\mathrm{SSR}}}
\label{eq:kappa_ssr}
\end{equation}

At the OD level, we multiply by the attenuation factor (not its reciprocal):

\begin{equation}
\Delta\text{OD}_{\text{SSR-attenuation corrected}}(\lambda) = (1 - R^2_{\mathrm{SS}}(\lambda))^{-1} \times \Delta\text{OD}_{\text{SSR-cleaned}}(\lambda)
\end{equation}

\begin{workexample}
Suppose at 760~nm the SSR regression explains $R^2_{\mathrm{SS}}(760) = 0.589$ of the variance in the long-channel signal (the value found in the real finger-tapping dataset analysed later). What is the attenuation and $\kappa_{\mathrm{SSR}}$?

\begin{align}
A_{\mathrm{SSR}}(760) &= 1 - 0.589 = 0.411 \\
\kappa_{\mathrm{SSR}}(760) &= \frac{1}{0.411} = 2.433
\end{align}

This means SSR removes about 58.9\% of the signal variance. The formula $1/(1-R^2_{\mathrm{SS}})$ treats \emph{all} of that removed variance as cortical signal to be restored.
\end{workexample}

\begin{keyinsight}
\textbf{Why we report $\kappa_{\mathrm{SSR}}$ but do not apply it.} Equation~\eqref{eq:kappa_ssr} assumes the entire variance SSR removed was cortical. In reality SSR removes mostly the \emph{superficial} (systemic) signal, which is exactly what it is supposed to discard. So $1/(1-R^2_{\mathrm{SS}})$ is an \emph{upper bound} on the restoration, not the true cortical loss. The cortical signal that survives SSR is already recovered, in full, by $\kappa_{\text{PV}}=1/f_{\text{cortex}}$ alone (because $f_{\text{cortex}}$ is defined on the post-SSR sensitivity). If we applied both $\kappa_{\mathrm{SSR}}$ \emph{and} the (correctly large) $\kappa_{\text{PV}}$, we would double-count the dilution and inflate the real-data response to $\sim$2.7~$\mu$M (above the physiological range). Therefore, in the final pipeline we \textbf{report $\kappa_{\mathrm{SSR}}$ as a diagnostic} of how much variance SSR removed (a flag for residual superficial contamination) but \textbf{apply only $\kappa_{\text{DPF}}$ and $\kappa_{\text{PV}}$}. This keeps the corrected amplitude physiological ($\sim$1.1~$\mu$M).
\end{keyinsight}

\subsection{Two correlations that look identical: $R$ vs.\ $\rho$}

A point that trips up almost everyone the first time: the regression statistic $R$ in $\kappa_{\mathrm{SSR}}=1/(1-R^2)$ is \emph{not} the same as the cortical--systemic correlation, even though both are computed with the identical Pearson formula. The formula is the same; the two signals you feed into it are different.
...
\begin{keyinsight}
This is exactly why $\kappa_{\mathrm{SSR}}=1/(1-R^2)$ over-restores. In real data $R^2\approx0.6$--$0.7$, but that is large because the long channel is \emph{systemic-dominated} (lots of $s$), not because SSR removed 60--70\% of the cortex. The cortex SSR actually nicks is governed by $\rho$ (small for a well-designed, externally paced task), so $1/(1-R^2)$ hands back far more than was lost, the double-counting we avoid by reporting $\kappa_{\mathrm{SSR}}$ as a diagnostic. If you remember one thing: \emph{$R$ is between the two channels you record; $\rho$ is between the two tissue signals you can't separate.}
\end{keyinsight}
...
\item \textbf{Partial volume correction} (at OD level): Divide by $f_{\text{cortex}}(\lambda)$
\item \textbf{SSR attenuation correction} (at OD level): Multiply by $\kappa_{\mathrm{SSR}}(\lambda)$
...
2. Wavelength-dependent corrections ($\kappa_{\text{PV}}(\lambda)$ and $\kappa_{\mathrm{SSR}}(\lambda)$) are separate at each wavelength
...
r_2 &= \sqrt{30^2 + (0 + 1 + 2 \times 1.94)^2} = \sqrt{900 + (4.88)^2} = \sqrt{900 + 23.81} = \sqrt{923.81} = 30.394~\text{mm}
\end{align}
...
This integral is evaluated numerically using Gaussian quadrature (e.g., scipy.integrate.quad in Python).
...
\begin{equation}
\kappa_{\text{PV}} = \frac{1}{f_{\text{cortex}}} \quad \text{mean across wavelengths} = \frac{3.456 + 3.594}{2} = 3.523
\end{equation}
...
At 760~nm: $\kappa_{\mathrm{SSR}}(760) = \frac{1}{1 - 0.128} = \frac{1}{0.872} = 1.147$

At 850~nm: $\kappa_{\mathrm{SSR}}(850) = \frac{1}{1 - 0.894} = \frac{1}{0.106} = 9.427$

The two wavelengths differ by nearly an order of magnitude ($1.147$ vs $9.427$), so \emph{no cross-wavelength mean is formed}: their average would not be a physically meaningful quantity. $\kappa_{\mathrm{SSR}}(\lambda)$ is reported per wavelength as a diagnostic only.

Applied correction (the $\kappa_{\mathrm{SSR}}$ diagnostics are \emph{not} multiplied in):
\begin{equation}
\kappa_{\text{applied}} = \kappa_{\text{DPF}} \times \kappa_{\text{PV}} = 1.000 \times 3.523 = 3.523
\end{equation}
...
\begin{tabular}{ccccc}
\hline
\textbf{SDS (mm)} & $f_{\text{cortex}}$ & $\kappa_{\text{PV}}$ & $\kappa_{\mathrm{SSR}}(760)$ & $\kappa_{\mathrm{SSR}}(850)$ \\
\hline
25 & 0.043 & 23.42 & 1.17 & 9.82 \\
30 & 0.064 & 15.54 & 1.19 & 10.15 \\
35 & 0.091 & 11.02 & 1.22 & 10.52 \\
40 & 0.110 & 9.12 & 1.21 & 10.52 \\
\hline
\end{tabular}
\end{center}

\paragraph{Note on $\kappa_{\mathrm{SSR}}$.} Across the study \emph{only $\kappa_{\text{PV}}$ (and $\kappa_{\text{DPF}}$) is applied to the data}; $\kappa_{\mathrm{SSR}}(\lambda)$ is reported \emph{per wavelength} as a diagnostic of residual superficial contamination, not multiplied into the corrected signal. It is listed separately at 760 and 850~nm because the two differ by nearly an order of magnitude, so no cross-wavelength mean (and no composite $\kappa_{\text{total}}$) is formed. This holds for both the synthetic validation and the real finger-tapping analysis later in this guide, for the reason given in the $\kappa_{\mathrm{SSR}}$ box above (applying it would double-count the dilution).

\subsection{Physical Interpretation}

\paragraph{Short separations (25 mm):} Very small cortical sensitivity ($\sim$4\%), requiring a very large partial-volume factor ($\kappa_{\text{PV}} = 23$). The long channel is dominated by superficial signal, so the SSR diagnostic is large at 850\,nm ($\kappa_{\mathrm{SSR}}(850)\approx9.8$). \emph{This is precisely the regime where the correction fails}: the cortical signal is so small it sits at or below the measurement-noise floor, and multiplying by 23 amplifies the noise. Short separations should be used for SSR, not for quantitative amplitude estimation.

\paragraph{Moderate-to-long separations (35--40 mm):} The cortical fraction rises to $\sim$9--11\%, so $\kappa_{\text{PV}}$ falls to $\sim$9--11 and, crucially, the cortical signal climbs above the noise floor. This is the regime where the correction is reliable (HbO$_2$ RMSE reduced by $\sim$44\% at 38--40~mm). The recommended operating point is SDS~$\geq$~38~mm.
...
HbR is corrected effectively at short-to-moderate separations (88.2\% at 25~mm, 84.7\% at 30~mm), but the benefit declines as SDS increases and becomes a slight \emph{worsening} at 40~mm ($-26.2\%$). This mirrors the HbO$_2$ pattern but is more pronounced for HbR: at long separations the cortical signal is already large (small $\kappa_{\text{PV}}$), so the correction adds little while the low-amplitude HbR channel is more easily dominated by amplified measurement noise. HbR therefore benefits most from correction precisely where partial-volume dilution is largest, at short separations.

\section{Wavelength Dependence of $\kappa_{\mathrm{SSR}}$}

A critical finding: $\kappa_{\mathrm{SSR}}$ differs significantly between wavelengths:

\begin{center}
\begin{tabular}{cccc}
\hline
\textbf{SDS (mm)} & $\kappa_{\mathrm{SSR}}(760)$ & $\kappa_{\mathrm{SSR}}(850)$ & \textbf{Ratio} \\
\hline
25 & 1.154 & 9.521 & 8.25 \\
30 & 1.147 & 9.427 & 8.22 \\
35 & 1.114 & 8.451 & 7.58 \\
...
\begin{itemize}
\item At 850~nm, where HbO$_2$ absorption dominates ($\varepsilon_{\mathrm{HbO_2}} > \varepsilon_{\mathrm{HbR}}$), the systemic signal accounts for a large fraction of the long-channel $\Delta\mathrm{OD}$, so the short channel explains most of its variance ($R^2_{\mathrm{SS}} \approx 0.83$, giving $\kappa_{\mathrm{SSR}} \approx 7$).
\item At 760~nm, where HbR absorption dominates ($\varepsilon_{\mathrm{HbR}} \gg \varepsilon_{\mathrm{HbO_2}}$), the short channel explains comparatively little of the long-channel variance ($R^2_{\mathrm{SS}} \approx 0.08$, giving $\kappa_{\mathrm{SSR}} \approx 1$).
\end{itemize}

Because the contamination structure differs so strongly between the two wavelengths, a single scalar $\bar{\kappa}_{\mathrm{SSR}}$ applied to both would misestimate the correction.

\textbf{Critical implication}: Applying a scalar average $\bar{\kappa}_{\mathrm{SSR}}$ to both wavelengths introduces error. The correct approach is to apply wavelength-specific $\kappa_{\mathrm{SSR}}(\lambda)$ at the OD level before spectroscopic inversion.
...
\subsection{Robustness to $f_{\text{cortex}}$ Errors}
...
\begin{keyinsight}
Even with $\pm 30\%$ error in $f_{\text{cortex}}$ (substantial anatomical uncertainty), the correction still improves HbO$_2$ RMSE by 35--66\% at long SDS (HbR is more sensitive, turning marginally negative only at the $-30\%$ extreme). Performance is best when $f_{\text{cortex}}$ is accurate and degrades on either side, faster for underestimates (where the inflated $\kappa_{\text{PV}}$ overcorrects and amplifies noise). The practical message: once a \emph{converged} forward model places $f_{\text{cortex}}$ in the right band, an approximate value is far better than none, so researchers who lack subject-specific MRI can still benefit from population-averaged anatomy, \emph{provided they work at long separations}.
\end{keyinsight}

\subsection{Three-Layer CSF Effects}

A more realistic three-layer model inserts a thin cerebrospinal fluid (CSF) layer between the skull and the cortex, matching the Monte Carlo used in the study:
\begin{itemize}
\item Layer 1 (scalp + skull): 10 mm, $\mu_a = 0.015$~mm$^{-1}$, $\mu_s' = 1.0$~mm$^{-1}$
\item Layer 2 (CSF): 2 mm, $\mu_a = 0.004$~mm$^{-1}$, $\mu_s' = 0.25$~mm$^{-1}$ (very low scattering)
\item Layer 3 (cortex): semi-infinite, $\mu_a = 0.020$~mm$^{-1}$, $\mu_s' = 0.80$~mm$^{-1}$
\end{itemize}
...
\begin{tabular}{ccccc}
\hline
\textbf{SDS (mm)} & $f_{\text{cortex}}^{\text{2L}}$ & $\gamma$ (MC) & $f_{\text{cortex}}^{\text{3L}}$ & Change \\
\hline
...
The Monte Carlo finds that the explicit CSF layer \emph{increases} $f_{\text{cortex}}$ by 40--80\% relative to the two-layer approximation ($\gamma = f^{\text{3L}}/f^{\text{2L}} \approx 1.4$--$1.8$, decreasing with SDS), in agreement with the light-piping literature.

\begin{caution}
\textbf{Warning: ignoring the CSF layer degrades (but at long SDS does not reverse) the correction.}

If the true tissue contains a CSF layer but corrections are computed with a two-layer model, the two-layer model \emph{underestimates} $f_{\text{cortex}}$ (e.g., $\approx$0.10 versus $\approx$0.15 at 38~mm). Its $\kappa_{\text{PV}} = 1/f_{\text{cortex}}$ is therefore too \emph{large} and partially overcorrects. At long separations the net effect is a reduced but still positive benefit (HbO$_2$ improvement $+34\%$ vs.\ $+70\%$ matched; HbR $+14\%$ vs.\ $+39\%$). The framework degrades gracefully rather than catastrophically, but including CSF (via the ratio $\gamma$) is recommended when anatomy is known.
\end{caution}
...
\section{Implementing $\kappa$-Correction in Your Lab}
...
For each SDS and anatomical configuration:

\begin{enumerate}
\item Solve the two-layer Kienle equation via Hankel transform
\item Compute fluence fields for source and detector
\item Integrate Jacobian over cortical and total volumes
\item Derive $f_{\text{cortex}}(\lambda)$ and $\kappa_{\text{PV}}(\lambda) = 1/f_{\text{cortex}}(\lambda)$
\end{enumerate}

This computation takes $\sim$0.1 s per channel on a modern laptop and can be done once, offline, before data acquisition.
...
\item Apply wavelength-specific $\kappa_{\text{PV}}$ at OD level
\item Apply wavelength-specific $\kappa_{\mathrm{SSR}}$ factor (if desired; alternative is to use simplified scalar approach)
...
\begin{caution}
\textbf{Pitfall 5: Not accounting for wavelength dependence in SSR correction.}

Applying a scalar $\bar{\kappa}_{\mathrm{SSR}}$ to both wavelengths ignores the 2--3$\times$ difference in $\kappa_{\mathrm{SSR}}$ between 760 and 850~nm.

\textbf{Fix}: Compute and apply wavelength-specific $\kappa_{\mathrm{SSR}}(\lambda)$ from the SSR regression coefficients.
\end{caution}
...
\item \textbf{Assumes task-evoked, not systemic signals}: If a cognitive task causes global blood pressure changes (task-evoked systemic response), SSR may remove some cortical signal. The $\kappa_{\mathrm{SSR}}$ diagnostic flags channels where this is likely (high $R^2_{\mathrm{SS}}$), but cannot by itself disentangle the ambiguity.
...
\item \textbf{$\kappa_{\text{SSR}}(\lambda)$ diagnostic:} The regression $R^2$ yields $\kappa_{\text{SSR}}(\lambda) = 1/(1 - R^2_{\text{SS}}(\lambda))$ ($760$: 2.433, $850$: 3.303 for this dataset). This is \emph{reported as a diagnostic of residual superficial contamination but not applied} (see the $\kappa_{\mathrm{SSR}}$ box earlier): applying it would double-count the dilution.
\item \textbf{Partial-volume ($\kappa_{\text{PV}}$) correction (the applied amplitude correction):} The two-layer Monte Carlo gives $f_{\text{cortex}}\approx0.10$ at 38~mm. But a \emph{real} adult head also contains a thin cerebrospinal-fluid (CSF) layer, which scatters light very weakly and ``pipes'' it down toward the cortex (see the CSF section). This raises the cortical fraction by the light-piping factor $\gamma\approx1.5$, to a three-layer value $f_{\text{cortex}}^{\mathrm{3L}}\approx0.15$. Because these are real heads, we use this CSF-augmented value, giving $\kappa_{\text{PV}}\approx6.5$ ($760$~nm: $1/0.153=6.56$; $850$~nm: $1/0.156=6.43$).
...
Applying $\kappa_{\text{PV}} = 6.5$ to the SSR-denoised data produces net amplification factors of 3.58$\times$ for HbO$_2$ (uncorrected peak $+0.265\,\mu$M $\rightarrow$ corrected $+0.95\,\mu$M) and 3.15$\times$ for HbR (uncorrected $-0.058\,\mu$M $\rightarrow$ corrected $-0.18\,\mu$M); across all five subjects the group-mean corrected HbO$_2$ is $1.13 \pm 0.41\,\mu$M ($4.6\times$). These magnitudes are physiologically plausible for a finger-tapping task. (Using the two-layer $f_{\text{cortex}}\approx0.10$ instead (ignoring CSF) gives the larger $\kappa_{\text{PV}}\approx9.7$ and $\approx1.7\,\mu$M, an upper bound; the physiological estimate is thus bracketed $\approx1.1$--$1.7\,\mu$M.) This is the payoff of two key choices: using a \emph{converged}, CSF-augmented $f_{\text{cortex}}$ (an under-converged $\approx 0.78$ would make $\kappa_{\text{PV}}$ do almost nothing), and treating $\kappa_{\text{SSR}}$ as a diagnostic (applying it as well would inflate the response to $\sim$2.7~$\mu$M by double-counting the dilution).

\subsection{Future Directions}

\begin{enumerate}
\item \textbf{Three-layer Kienle solution} with CSF layer for full anatomical accuracy.
\item \textbf{Hybrid CSF correction}: A particularly promising approach is to compute $f_{\text{cortex}}$ rapidly using the two-layer Hankel transform ($\sim$0.1~s per channel) and then multiply by an SDS-dependent correction factor $\gamma(d_{\text{SDS}}, d_{\text{CSF}})$ from a precomputed lookup table. The adjusted sensitivity $f_{\text{cortex}}^{\text{adj}} = \gamma \times f_{\text{cortex}}^{\text{2L}}$ would retain the speed of the analytical two-layer solution while accounting for the CSF light-guiding effect. Our three-layer analysis shows that $\gamma$ varies systematically: approximately 2.3 at 25~mm, 2.0 at 30~mm, 1.8 at 35~mm, and 1.6 at 40~mm SDS.
...
\textit{Solution:} (a)~$0.10\times1.5=0.15$. (b)~$\kappa_{\text{PV}}$ drops from $1/0.10=10$ to $1/0.15\approx6.7$. (c)~Down: a smaller $\kappa_{\text{PV}}$ multiplies the signal by less, so the recovered amplitude decreases (in the paper, $\approx1.6\to\approx1.1~\mu$M).
...
\textit{Solution:} Band-pass the optical density; regress out the short channel (SSR) to remove systemic contamination; multiply the SSR-cleaned $\Delta$OD by $\kappa_{\text{PV}}=1/f_{\text{cortex}}$ (CSF-augmented) at each wavelength to undo partial-volume dilution; invert with the MBLL using wavelength-specific DPFs. $\kappa_{\text{SSR}}$ is computed and reported as a diagnostic of residual contamination but \emph{not} applied. The result is trustworthy only at long SDS ($\geq$38~mm), because there the cortical signal is above the noise floor; at short SDS the same correction amplifies noise.
...
$f_{\text{cortex}}$ & Cortical sensitivity fraction & Dimensionless (0.1--0.9) \\
\hline
$\kappa_{\text{DPF}}$ & DPF correction factor & Dimensionless ($\sim 1$) \\
$\kappa_{\text{PV}}$ & Partial volume correction factor & Dimensionless (1.1--7.5) \\
$\kappa_{\mathrm{SSR}}(\lambda)$ & SSR attenuation correction (wavelength-specific) & Dimensionless (1.5--10) \\
$\kappa_{\text{applied}}$ & Applied composite, $\kappa_{\text{DPF}}\times\kappa_{\text{PV}}$ ($\kappa_{\mathrm{SSR}}$ is a per-wavelength diagnostic, not applied) & Dimensionless \\
\hline
