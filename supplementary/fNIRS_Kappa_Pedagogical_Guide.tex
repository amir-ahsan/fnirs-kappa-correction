...
The neural task-evoked response of interest originates in the active cortex. The superficial layers (scalp, skull) show continuous systemic physiology (cardiac pulsations, respiration, blood pressure oscillations) and can additionally carry \emph{task-correlated} extracerebral responses (e.g., task-evoked changes in scalp blood flow)---which is exactly why short-separation regression is used to estimate and remove the superficial contribution rather than assuming the scalp is inert.
...
\begin{tabular}{cccc}
\hline
\textbf{SDS (mm)} & $f_{\text{cortex}}$ (760\,nm) & $\kappa_{\text{PV}}=1/f_{\text{cortex}}$ & \textbf{What this means} \\
\hline
25 & 0.042 & 24 & Only $\sim$4\% of signal from cortex \\
30 & 0.064 & 16 & Only $\sim$6\% from cortex \\
35 & 0.088 & 11 & Only $\sim$9\% from cortex \\
40 & 0.110 & 9 & $\sim$11\% from cortex \\
\hline
\end{tabular}
\end{center}

These values come from a converged Monte-Carlo light-transport simulation of the two-layer head. They are the \emph{760\,nm} production fractions (kept single-wavelength here so the simple 4\%, 6\%, 9\%, 11\% examples stay clean; the wavelength-averaged fractions used elsewhere appear in the later Summary Table). They occupy the same low-sensitivity regime as published whole-head ``atlas'' Monte Carlo but are systematically lower---especially at short SDS---so the comparison provides qualitative external consistency (and confirms the order of magnitude of the correction), not an exact quantitative match. They are an order of magnitude smaller than an under-resolved analytical calculation suggests (a subtle numerical pitfall we discuss in the advanced sections). Since MBLL assumes all tissue is homogeneous, it treats the entire measured OD as coming from a single concentration change, so its estimate is attenuated by $f_{\text{cortex}}$. At 25~mm SDS, MBLL reports only $\sim$4\% of the true cortical concentration change---an underestimation by a factor of $\sim$23.
...
\paragraph{Physical intuition.} OD~$=1$ means the detected intensity is 10\% of the reference value (a tenfold attenuation of the detected light); OD~$=2$ means 1\%. Each unit of OD reduces the detected intensity by a factor of 10. (In turbid tissue this attenuation reflects both absorption and scattering---including photons scattered out of the detected path---not absorption alone.)
...
\begin{itemize}
\item \textbf{Task-evoked signals.} The neural task-evoked response of interest originates in cortex, so $\Delta c_{\text{cort}} \neq 0$. Superficial tissues, however, can exhibit systemic \emph{and} task-correlated extracerebral responses (e.g., task-evoked changes in scalp blood flow), so in general $\Delta c_{\text{sup}}$ need not be zero. Setting $\Delta c_{\text{sup}} \approx 0$ in the idealized derivation below is a \emph{pedagogical} simplification used to isolate the partial-volume attenuation term; in real data, short-separation regression (SSR) is used to estimate and reduce the superficial contribution rather than assuming it away.
...
In the idealized case $\Delta c_{\text{sup}} \approx 0$, equation (\ref{eq:od_layers}) reduces to:
...
and the superficial term drops out. This is the clean case that isolates the partial-volume factor; when $\Delta c_{\text{sup}} \neq 0$ (the realistic situation), SSR is applied first to suppress the superficial contribution before the partial-volume correction.
...
In the idealized case where the superficial term is negligible (or has already been suppressed by SSR), the true cortical concentration is recovered by dividing the measured OD by $f_{\text{cortex}}$:
...
\begin{workexample}
Suppose $f_{\text{cortex}}(760) = 0.064$ at 30~mm SDS (the converged Monte-Carlo value). What is $\kappa_{\text{PV}}$?

\begin{equation}
\kappa_{\text{PV}}(760) = \frac{1}{0.064} = 15.6
\end{equation}

This means that MBLL underestimates the cortical HbO$_2$ change by a factor of $\sim$16. If the true cortical change is 2.5~$\mu$M, MBLL reports only $2.5 / 15.6 \approx 0.16$~$\mu$M. Multiplying by $\kappa_{\text{PV}} = 15.6$ recovers the true value, provided that tiny 0.16~$\mu$M signal is itself above the measurement noise, which (as we will see) it may not be at short separations.
\end{workexample}
...
Now consider the real fNIRS case at 30~mm SDS, where $f_{\text{cortex}} = 0.064$. This is like a group of \emph{sixteen} students where only one studied: just $\sim$6\% of the answers come from the diligent student, and the rest are random guesses. The observed group score might be only $6/100$, and the correction factor $\kappa_{\text{PV}} = 1/0.064 \approx 16$ would recover the true individual score: $6 \times 16 \approx 100$. But notice the catch: if the diligent student's $6$ points are barely distinguishable from the lucky guesses of the others (the ``noise''), multiplying by 16 also multiplies that ambiguity. This is exactly why, at short separations where the cortical contribution is so small, the correction can amplify noise instead of signal, and why we apply it only at long separations where the cortical share is larger ($\sim$11\% at 40~mm, $\kappa_{\text{PV}}\approx9$).
...
\end{align}
The interpretation is a \emph{report}: ``at 760~nm the short channel removed about 44\% of the long-channel variance, leaving a residual standard deviation (RMS fluctuation) about 75\% of the original.'' At 850~nm the short-channel regressor explains about 57\% of the long-channel variance, giving $R^2_{\mathrm{SS}}(850)\approx0.57$ and $V_{\mathrm{SSR}}(850)\approx2.34$ --- a much larger diagnostic value (the composition of that explained variance cannot be identified from $R^2$ alone), which is precisely why $V_{\mathrm{SSR}}$ is reported \emph{per wavelength} and never averaged across wavelengths. Neither number is multiplied into the recovered concentration.
\end{workexample}
...
\begin{keyinsight}
This is exactly why $V_{\mathrm{SSR}}=1/(1-R^2)$ must \emph{not} be read as an amount of cortex to hand back. In real data $R^2\approx0.4$--$0.6$; in this constructed two-component example that is large chiefly because the long channel is \emph{systemic-dominated} (lots of $s$), not because SSR removed that fraction of the cortex. The cortex SSR actually removes is \emph{not} set by $R^2$: it depends on the cortical--systemic covariance ($\rho$), the relative signal amplitudes, the short-channel composition and any cortical sensitivity it carries, the preprocessing, and the regression model (and is typically small for a well-designed, externally paced task). A hypothetical $1/(1-R^2)$ amplitude restoration would therefore hand back far more than was lost --- which is precisely why we report $V_{\mathrm{SSR}}$ as a variance diagnostic and never apply it. If you remember one thing: \emph{$R$ is between the two channels you record; $\rho$ is between the two tissue signals you can't separate.}
\end{keyinsight}
...
\begin{itemize}
\item $f_{\text{cortex}}(760) = 0.064$, $f_{\text{cortex}}(850) = 0.063$ (converged Monte-Carlo values; wavelengths have slightly different sensitivities)
...
\begin{align}
\Delta\text{OD}(760) &= 0.064 \times [\text{extinction terms}] \\
\Delta\text{OD}(850) &= 0.063 \times [\text{extinction terms}]
\end{align}

Apply partial volume correction:
\begin{align}
\Delta\text{OD}_{\text{corrected}}(760) &= \frac{\Delta\text{OD}(760)}{0.064} = [\text{extinction terms}] \\
\Delta\text{OD}_{\text{corrected}}(850) &= \frac{\Delta\text{OD}(850)}{0.063} = [\text{extinction terms}]
\end{align}
...
Now, to apply correction, one might compute an average $\kappa_{\text{PV}} = (15.6 + 15.8)/2 \approx 15.7$ and multiply both by this scalar. But this is wrong because HbO$_2$ and HbR have different wavelength dependencies:
...
\begin{keyinsight}
\textbf{A numerical cautionary tale.} The two-layer Jacobian is computed via a Hankel (Bessel) transform, evaluated as a sum over spatial frequencies $s$. The integrand contains a term $e^{\alpha(s)\,z}$ that \emph{grows} with $s$; at the large $s$ needed to resolve the source/detector, it overflows and is then cancelled against a decaying term. The fluence itself is fine, but the depth-weighted \emph{ratio} $f_{\text{cortex}}$ is not: as you add more quadrature points it drifts ($0.0005 \to 0.62 \to 0.29 \to 0.19 \to 0.10 \to 0.085$\,\ldots). The sequence moves toward the Monte Carlo sensitivity band ($\sim$0.06--0.09 at this geometry) as resolution increases, but the supplied fixed-point calculation has \emph{not} been demonstrated to be quadrature-converged, so this trend is suggestive rather than a proof that the analytical kernel converges to the Monte Carlo result. An ``apparently converged'' value at a fixed coarse setting (e.g.\ $0.285$) can be an order of magnitude too high. The lesson: always check convergence of the \emph{quantity you actually report}, not just the underlying field, and cross-check an analytical kernel against Monte Carlo. We use a converged two-layer Monte Carlo (\texttt{mc\_2layer.py}) for all $f_{\text{cortex}}$ values in this guide.
\end{keyinsight}
...
\begin{equation}
\text{HRF}(t) = \frac{t^{a_1-1} e^{-t/b_1}}{b_1^{a_1} \Gamma(a_1)} - c \, \frac{t^{a_2-1} e^{-t/b_2}}{b_2^{a_2} \Gamma(a_2)}
\end{equation}

Parameters: $a_1 = 6$, $b_1 = 1$~s, $a_2 = 16$, $b_2 = 1$~s, $c = 0.35$ (matching the principal synthetic validation in \texttt{fnirs\_kappa\_synthetic\_validation.py} and the main manuscript; the simplified \texttt{fnirs\_invivo\_demo.py} walkthrough uses a shallower undershoot, $c = 0.1$).

This produces an HRF that rises over 4--6 seconds, peaks around 5 seconds (the first gamma term with $a_1=6$, $b_1=1$~s peaks at $t=(a_1-1)b_1=5$~s), and then decays over 15--20 seconds.
...
Compute the regression coefficient $\beta(\lambda)$ by minimizing the residual variance. The fraction of long-channel variance removed by (coupled to) the short-channel regression is the ordinary $R^2$:
...
\begin{itemize}
\item $R^2_{\mathrm{SS}}(760) = 0.017$
\item $R^2_{\mathrm{SS}}(850) = 0.593$
\end{itemize}

These values indicate that SSR removes about 2\% of the long-channel variance at 760\,nm and about 59\% at 850\,nm. The removed variance reflects short-channel-coupled long-channel variance (systemic variance, cortical--systemic covariance, noise, and any short-channel cortical sensitivity), not solely superficial contamination.
...
At 30~mm SDS: $f_{\text{cortex}}(760) = 0.064$, $f_{\text{cortex}}(850) = 0.063$ (converged Monte Carlo).

\begin{equation}
\kappa_{\text{PV}}(760) = \frac{1}{0.064} = 15.6, \quad \kappa_{\text{PV}}(850) = \frac{1}{0.063} = 15.8
\end{equation}
...
\begin{equation}
\kappa_{\text{PV}} = \frac{1}{f_{\text{cortex}}} \quad \text{mean across wavelengths} = \frac{15.6 + 15.8}{2} = 15.7
\end{equation}
...
At 760~nm: $V_{\mathrm{SSR}}(760) = \frac{1}{1 - 0.017} = \frac{1}{0.983} = 1.02$

At 850~nm: $V_{\mathrm{SSR}}(850) = \frac{1}{1 - 0.593} = \frac{1}{0.407} = 2.46$

Here $R^2_{\mathrm{SS}}(\lambda)$ is the \emph{ordinary} regression $R^2$ of the uncorrected long channel on the short channel (the fraction of long-channel variance removed by SSR). The same variance-removal definition is used in the synthetic and real-data pipelines, but the preprocessing and regression implementations differ: the synthetic analysis uses unfiltered OD with an intercept, whereas the in-vivo analysis uses 0.01--0.1\,Hz bandpass-filtered, approximately zero-mean OD with a zero-intercept regression. The two wavelengths differ substantially ($1.02$ vs $2.46$, roughly $2.4\times$), so \emph{no cross-wavelength mean is formed}: their average would not be a physically meaningful quantity. $V_{\mathrm{SSR}}(\lambda)$ is reported per wavelength as a variance-removal diagnostic only.
...
\hline
25 & 0.043 & 23.50 & 1.01 & 2.40 \\
30 & 0.064 & 15.69 & 1.02 & 2.46 \\
35 & 0.086 & 11.57 & 1.02 & 2.48 \\
38 & 0.100 & 10.04 & 1.02 & 2.47 \\
40 & 0.108 & 9.22 & 1.02 & 2.46 \\
\hline
\end{tabular}
\end{center}

\noindent (The $f_{\text{cortex}}$ column is the \emph{wavelength-averaged} fraction, i.e.\ the mean of the 760 and 850~nm converged Monte-Carlo values from \texttt{fcortex\_production.json}, so that $\kappa_{\text{PV}}=1/f_{\text{cortex}}$ in the next column is its exact reciprocal; the pipelines themselves use the wavelength-specific fractions. See the single-source-of-truth note in Appendix~A.)

\paragraph{Note on $V_{\mathrm{SSR}}$.} Across the study \emph{only $\kappa_{\text{PV}}$ (and $\kappa_{\text{DPF}}$) is applied to the data}; $V_{\mathrm{SSR}}(\lambda)$ is reported \emph{per wavelength} as a variance-removal diagnostic (of short-channel-coupled long-channel variance and the risk of cortical co-removal, \emph{not} a measure of superficial contamination remaining after SSR), never multiplied into the corrected signal. It is listed separately at 760 and 850~nm because the two differ substantially (roughly $2$--$2.5\times$), so no cross-wavelength mean (and no composite $\kappa_{\text{total}}$) is formed. This holds for both the synthetic validation and the real finger-tapping analysis later in this guide, for the reason given in the $V_{\mathrm{SSR}}$ box above ($R^2_{\mathrm{SS}}$ cannot identify the cortical component of the removed variance).

\subsection{Physical Interpretation}

\paragraph{Short separations (25 mm):} Very small cortical sensitivity ($\sim$4\%), requiring a very large partial-volume factor ($\kappa_{\text{PV}} = 23$). The long channel is dominated by superficial signal, so the SSR diagnostic is larger at 850\,nm ($V_{\mathrm{SSR}}(850)\approx2.40$) than at 760\,nm. \emph{This is precisely the regime where the correction fails}: the cortical signal is so small it sits at or below the measurement-noise floor, and multiplying by 23 amplifies the noise. Short separations should be used for SSR, not for quantitative amplitude estimation.
...
\begin{itemize}
\item At 850~nm, where HbO$_2$ absorption dominates ($\varepsilon_{\mathrm{HbO_2}} > \varepsilon_{\mathrm{HbR}}$), the systemic signal accounts for a larger fraction of the long-channel $\Delta\mathrm{OD}$, so the short channel explains more of its variance ($R^2_{\mathrm{SS}} \approx 0.59$, giving $V_{\mathrm{SSR}} \approx 2.45$).
\item At 760~nm, where HbR absorption dominates ($\varepsilon_{\mathrm{HbR}} \gg \varepsilon_{\mathrm{HbO_2}}$), the short channel explains comparatively little of the long-channel variance ($R^2_{\mathrm{SS}} \approx 0.02$, giving $V_{\mathrm{SSR}} \approx 1.02$).
\end{itemize}
...
8 & 0.2192 & 4.56 & $-$72.8\% \\
10 & 0.1226 & 8.15 & $-$51.3\% \\
12 & 0.05973 & 16.74 & 0.0\% (baseline) \\
14 & 0.03136 & 31.89 & $+$90.4\% \\
16 & 0.01676 & 59.66 & $+$256.3\% \\
\hline
\end{tabular}
\end{center}

These are the current converged Monte-Carlo values read directly from \texttt{results/\allowbreak robustness\_\allowbreak secondary.json} (the ``\% Change'' column is the change in $\kappa_{\text{PV}}$ relative to the 12~mm baseline). The dependence is strong and \emph{asymmetric}: a $+$2~mm error (12$\rightarrow$14~mm) raises $\kappa_{\text{PV}}$ by $\approx+$90\% (and lowers $f_{\text{cortex}}$ by $\approx-$47\%), while a $-$2~mm error (12$\rightarrow$10~mm) lowers $\kappa_{\text{PV}}$ by $\approx-$51\% (and raises $f_{\text{cortex}}$ by $\approx+$105\%). Superficial-layer thickness is the single most important input to the correction, strongly motivating subject-specific anatomy (MRI, ultrasound, or an age-matched atlas).
...
\begin{tabular}{ccc}
\hline
\textbf{Property Change} & $\mu_a$ effect on $\kappa_{\text{PV}}$ & $\mu_s'$ effect on $\kappa_{\text{PV}}$ \\
\hline
$-30\%$ & $-34.6\%$ & $-17.2\%$ \\
Nominal & 0\% & 0\% \\
$+30\%$ & $+40.1\%$ & $+12.5\%$ \\
...
\noindent (Values from \texttt{results/robustness\_secondary.json}; $\mu_a$ and $\mu_s'$ are perturbed \emph{separately}, so no combined-perturbation column is shown.)

Absorption is the dominant optical input (because $\kappa_{\text{PV}} = 1/f_{\text{cortex}}$ tracks the survival-weighted cortical pathlength). Optical-property uncertainty is therefore not negligible ($\mu_a$ dominates, at $\approx-$35\% to $+$40\% in $\kappa_{\text{PV}}$ for $\pm$30\%), but it is still smaller than the effect of superficial-layer thickness ($\approx+$90\% in $\kappa_{\text{PV}}$ for a $+2$~mm error, and $\approx-$51\% for a $-2$~mm error, above), which remains the single most important input. Note that an earlier, naively evaluated analytical kernel understated this optical sensitivity (it suggested only $\pm 13$--$15\%$); the converged Monte Carlo is the reliable estimate.
...
\begin{keyinsight}
Even with $\pm 30\%$ error in $f_{\text{cortex}}$ (a \emph{moderate} generic modeling perturbation---note that a realistic $\pm 2$~mm superficial-depth error can push $f_{\text{cortex}}$ much further, to about $+105\%$, and should be assessed with the thickness-sensitivity table above rather than this generic $\pm 30\%$ test), the correction still improves HbO$_2$ RMSE by 34--66\% at long SDS (HbR is more sensitive, turning marginally negative only at the $-30\%$ extreme). Performance is best when $f_{\text{cortex}}$ is accurate and degrades on either side, faster for underestimates (where the inflated $\kappa_{\text{PV}}$ overcorrects and amplifies noise). The practical message: once a \emph{converged} forward model places $f_{\text{cortex}}$ in the right band, an approximate value is far better than none, so researchers who lack subject-specific MRI can still benefit from population-averaged anatomy, \emph{provided they work at long separations}.
\end{keyinsight}
...
\end{itemize}
(The total superficial depth is held at 12~mm so the cortical boundary is unchanged; a 1~mm CSF thickness check is also emitted, giving a smaller $\gamma\approx1.25$--$1.44$ across the full 25--42~mm two-wavelength grid.)
...
42 & 0.123 & 1.37 & 0.168 & $+$37\% \\
\hline
\end{tabular}
\end{center}

The Monte Carlo finds that the explicit CSF layer \emph{increases} $f_{\text{cortex}}$ by $\approx$37--84\% at 760~nm across the full 25--42~mm grid relative to the two-layer approximation ($\gamma = f^{\text{3L}}/f^{\text{2L}} \approx 1.37$--$1.84$, decreasing with SDS), in agreement with the light-piping literature.
...
\begin{caution}
\textbf{Modelling note.} In the current production study the ratio $\gamma$ is computed with the \emph{same} anisotropic Henyey--Greenstein ($g=0.9$) transport in both the two- and three-layer runs (\texttt{mc\_production.py}); an earlier \emph{isotropic}-scattering CSF Monte Carlo is retained only as a legacy cross-check, and the two agree within Monte-Carlo uncertainty, confirming the ratio is robust to the scattering-anisotropy assumption. We apply $\gamma$ to the converged two-layer fractions ($f^{\text{3L}}=\gamma f^{\text{2L}}$). Even with light-piping the cortical fraction stays small ($\lesssim$0.17; it reaches $\approx$0.168 at 42~mm), so $\kappa_{\text{PV}}$ remains large. A further modelling boundary condition: the Monte Carlo applies Fresnel reflection/refraction \emph{only} at the external tissue--air surface; the internal scalp/CSF/cortex interfaces are treated as refractive-index matched (all layers $n=1.4$). The $n\approx1.33$ (CSF) / $n\approx1.40$ (surrounding tissue) values quoted for the anatomy are the physical indices; the present Monte Carlo does \emph{not} apply an internal $n_{\text{CSF}}\approx1.33$ Fresnel mismatch, and $\gamma$ isolates the CSF scattering/absorption (light-piping) contrast. Adding the internal index step is a possible future sensitivity analysis. For reference, an earlier diffusion finite-difference estimate gave a spurious $\sim$99\% \emph{reduction}; that was a numerical artifact, because diffusion theory fails in the near-transparent CSF layer, which is exactly why Monte Carlo is needed here.
\end{caution}
...
Assuming $d_{\text{sup}} = 8$~mm when the true value is 12~mm would \emph{under}-scale $\kappa_{\text{PV}}$ by about 73\% ($\kappa_{\text{PV}}\approx4.56$ at 8~mm versus $\approx16.74$ at 12~mm; see the thickness-sensitivity table above)---a large error, underscoring how sensitive the correction is to the assumed superficial-layer depth.
...
Application of the pipeline to the publicly available BIDS-NIRS-Tapping finger-tapping dataset (Luke et al., 2021) demonstrates the framework on real data as a proof of concept. There is no concurrent ground truth here, so this tests whether the recovered amplitudes are physiologically plausible, not whether they are quantitatively accurate. The retained contralateral channels span source--detector separations of 33.4--40.9~mm (centered near 38~mm, median 37.2~mm): most lie within or above the beneficial $\sim$35~mm long-separation regime, with a minority (ten channels below 35~mm) near its lower, noise-dependent transition, which the main manuscript examines separately. The pipeline proceeds in ordered stages at the OD level:
...
Applying the per-channel $\kappa_{\text{PV}}$ to the SSR-denoised data recovers physiologically plausible amplitudes. For Subject~01 (contralateral, window-mean) the uncorrected HbO$_2$ of $+0.144\,\mu$M rises to $+0.646\,\mu$M ($\kappa_{\text{PV}}=6.68$) and HbR from $-0.030$ to $-0.179\,\mu$M; across all five subjects the group-mean corrected HbO$_2$ is $0.913 \pm 0.334\,\mu$M ($\sim$7$\times$ the uncorrected $0.124\,\mu$M) and HbR $-0.047\rightarrow-0.341\,\mu$M. These magnitudes are physiologically plausible for a finger-tapping task. (Using the two-layer $f_{\text{cortex}}\approx0.10$ instead (ignoring CSF) gives the larger $\kappa_{\text{PV}}\approx10.0$ and $\approx1.4\,\mu$M, an upper bound; whether a CSF layer is included at all thus brackets the estimate at $\approx0.9$--$1.4\,\mu$M, while the narrower 1--2\,mm CSF-thickness range spans only $\approx0.9$--$1.0\,\mu$M.) This $\approx0.9$--$1.4\,\mu$M interval is specifically a \emph{CSF-model bracket}, \emph{not} a comprehensive forward-model uncertainty: it does not include scalp/skull-to-cortex (superficial-depth) uncertainty, which is the dominant anatomical input and---per the thickness-sensitivity table above---could shift $f_{\text{cortex}}$ (and hence the amplitude) by more than the CSF-model choice does. It was not propagated here because these participants had no subject-specific anatomy. This is the payoff of using a \emph{converged}, CSF-augmented $f_{\text{cortex}}$: an under-converged $\approx0.78$ would make $\kappa_{\text{PV}}$ do almost nothing. Consistent with the statistical clarification, $V_{\text{SSR}}$ is reported as a variance diagnostic and is \emph{not} applied.
...
\textit{Solution:} No. $R^2_{\text{SS}}=0.64$ means the short-channel regression explains 64\% of the \emph{long-channel} variance. It does not identify the composition of that explained variance or tell us what fraction was cortical. The cortical loss is not set by $R^2$; it depends on the cortical--systemic covariance, the signal amplitudes, the short-channel composition, the preprocessing, and the regression model, and is usually much smaller. This is exactly why $V_{\text{SSR}}=1/(1-R^2)$ is reported as a variance-removal diagnostic and never applied as an amplitude restoration.
...
\noindent The refractive indices above are the \emph{physical} layer values. The production Monte Carlo applies Fresnel reflection/refraction only at the external tissue--air surface and treats all internal boundaries as refractive-index matched ($n=1.4$); it does \emph{not} model the internal $n_{\text{CSF}}\approx1.33$ step (see the CSF ``Modelling note''). Including that internal mismatch is left as a future sensitivity analysis.

\subsection{Monte-Carlo settings and the single source of truth}

All \emph{principal} baseline and CSF cortical-sensitivity values used in this study --- the two-layer $f_{\text{cortex}}$, the CSF light-piping ratios $\gamma(\lambda,\text{SDS})$, and the derived $\kappa_{\text{PV}}$ --- come from \emph{one} converged production Monte Carlo, written to a single versioned file that both the synthetic and the real-data pipelines read:
\begin{center}\small
...
\end{center}
The only cortical-sensitivity values \emph{not} taken from this file are the two \emph{secondary} robustness sweeps (superficial-thickness and cortical-$\mu_a$/$\mu_s'$ perturbations), which by construction move the geometry/optics away from the production configuration and are produced by a separate versioned artifact (\texttt{mc\_robustness\_sweeps.py} $\rightarrow$ \texttt{results/robustness\_secondary.json}, payload-hash verified; in release mode it is mandatory, and the archived constant arrays are a development-only fallback). No principal analysis script carries a hard-coded sensitivity table or a $760\rightarrow850$ ratio shortcut. The production settings are: anisotropic Henyey--Greenstein scattering $g=0.9$ (with $\mu_s=\mu_s'/(1-g)$), a Fresnel surface, partial-pathlength scoring, $N=2\times10^{6}$ photons per (geometry, wavelength), a converged photon-pathlength cutoff $L_{\max}=1200$~mm and domain depth $z_{\max}=150$~mm (both selected from an observed numerical plateau), and seed~$=1$. Uncertainty on each $f_{\text{cortex}}$ is the spread of the \emph{ratio estimator} across 16 independent, \emph{launch-defined} photon batches (batch id $=$ launch index mod 16, not a seed-level bootstrap); keying the batches by launch index makes batch $b$ draw on the same launched-photon cohort across geometries, so the CSF ratio $\gamma_b=f^{3L}_b/f^{2L}_b$ can be formed batchwise as a \emph{launch-index-matched} cross-check (a launch-index match, not a strict per-photon common-random-number stream, since the vectorized transport diverges once the geometries' active sets differ). The \emph{primary} reported $\gamma$ uncertainty is the zero-covariance propagated standard error; the launch-index-matched batch-ratio SE is reported as a companion cross-check, and neither estimator is uniformly larger across the calibrated grid; the file also stores per-SDS DPF and $N_{\text{eff}}$ (an absorption-weight count only), the wavelength-specific CSF ratios $\gamma(\lambda,\text{SDS})$ at 2~mm and 1~mm CSF thickness, and an $L_{\max}$/$z_{\max}$/annulus convergence sweep. The exact per-layer wavelength-specific optical properties used by the MC are stored in the \texttt{fcortex\_production.json} metadata (760~nm: scalp $\mu_a{=}0.0128$, $\mu_s'{=}1.08$; cortex $\mu_a{=}0.0192$, $\mu_s'{=}0.82$; 850~nm: scalp $\mu_a{=}0.0165$, $\mu_s'{=}0.86$; cortex $\mu_a{=}0.0212$, $\mu_s'{=}0.76$~mm$^{-1}$; CSF $\mu_a{=}0.004$, $\mu_s'{=}0.25$/0.24~mm$^{-1}$).
...
$V_{\mathrm{SSR}}(\lambda)$ & SSR variance-removal diagnostic, $1/(1-R^2_{\mathrm{SS}})$ (reported, never applied) & Dimensionless ($\sim$1--3 in this study: synthetic $\approx$1.0--2.5, experimental $\approx$1.6--2.8) \\
...
\item Sagara N, Ahsan A. Partial-volume correction in continuous-wave fNIRS: operating regime, a Monte-Carlo convergence caveat, and a reproducible pipeline. \textit{Neurophotonics}. 2026 (submitted).
\begin{quote}
