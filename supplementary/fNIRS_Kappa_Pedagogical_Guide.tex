...
\item \textbf{Hybrid CSF correction (recommended practical default)}: compute $f_{\text{cortex}}$ rapidly using the two-layer Hankel transform ($\sim$0.1~s per channel) and then multiply by an SDS-dependent, \emph{Monte-Carlo-calibrated} correction factor $\gamma(d_{\text{SDS}}, d_{\text{CSF}})$ from a precomputed lookup table. The adjusted sensitivity $f_{\text{cortex}}^{\text{adj}} = \gamma \times f_{\text{cortex}}^{\text{2L}}$ retains the speed of the analytical two-layer solution while supplying the CSF light-guiding effect from Monte Carlo (not diffusion), used within the calibrated range (SDS 25--40~mm, CSF 1--2~mm, 760/850~nm, adult slab). Our three-layer analysis shows that $\gamma$ varies systematically: approximately 1.84 at 25~mm, 1.66 at 30~mm, 1.57 at 35~mm, and 1.44 at 40~mm SDS (760~nm).
...
\]
No analysis script carries a hard-coded sensitivity table or a $760\rightarrow850$ ratio shortcut. The production settings are: anisotropic Henyey--Greenstein scattering $g=0.9$ (with $\mu_s=\mu_s'/(1-g)$), a Fresnel surface, partial-pathlength scoring, $N=2\times10^{6}$ photons per (geometry, wavelength), a converged photon-pathlength cutoff $L_{\max}=1200$~mm and domain depth $z_{\max}=150$~mm (both selected from an observed numerical plateau), and seed~$=1$. Uncertainty on each $f_{\text{cortex}}$ is the spread of the \emph{ratio estimator} across 16 independent, \emph{launch-defined} photon batches (batch id $=$ launch index mod 16, not a seed-level bootstrap); keying the batches by launch index makes batch $b$ the same launched-photon cohort across geometries, so the CSF ratio $\gamma_b=f^{3L}_b/f^{2L}_b$ and the $z_{\max}$ difference are computed as genuinely \emph{paired} per-batch statistics (common random numbers), with an independent-propagation standard error also reported as a conservative cross-check; the file also stores per-SDS DPF and $N_{\text{eff}}$ (an absorption-weight count only), the wavelength-specific CSF ratios $\gamma(\lambda,\text{SDS})$ at 2~mm and 1~mm CSF thickness, and an $L_{\max}$/$z_{\max}$/annulus convergence sweep. The exact per-layer wavelength-specific optical properties used by the MC are stored in the \texttt{fcortex\_production.json} metadata (760~nm: scalp $\mu_a{=}0.0128$, $\mu_s'{=}1.08$; cortex $\mu_a{=}0.0192$, $\mu_s'{=}0.82$; 850~nm: scalp $\mu_a{=}0.0165$, $\mu_s'{=}0.86$; cortex $\mu_a{=}0.0212$, $\mu_s'{=}0.76$~mm$^{-1}$; CSF $\mu_a{=}0.004$, $\mu_s'{=}0.25$/0.24~mm$^{-1}$).
