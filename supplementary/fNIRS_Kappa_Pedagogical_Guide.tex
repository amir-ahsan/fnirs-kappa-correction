...
\subsection{Factor 3: $V_{\text{SSR}}(\lambda)$, the SSR Variance-Removal Diagnostic (reported, not applied)}
\label{sec:vssr}
...
\text{amplitude ratio} &= \sqrt{1-0.44} \approx 0.75 .
\end{align}
...
\section{Wavelength Dependence of $V_{\mathrm{SSR}}$}
...
$V_{\mathrm{SSR}}(\lambda)=1/(1-R^2_{\mathrm{SS}})$ is an inverse residual-\emph{variance} ratio, not an amplitude factor (the amplitude ratio is $\sqrt{1-R^2_{\mathrm{SS}}}$), and its value differs by nearly an order of magnitude between 760 and 850~nm, so a cross-wavelength average is meaningless.
...
Applying the per-channel $\kappa_{\text{PV}}$ to the SSR-denoised data recovers physiologically plausible amplitudes. For Subject~01 (contralateral, window-mean) the uncorrected HbO$_2$ of $+0.144\,\mu$M rises to $+0.649\,\mu$M ($\kappa_{\text{PV}}=6.71$) and HbR from $-0.030$ to $-0.181\,\mu$M; across all five subjects the group-mean corrected HbO$_2$ is $0.92 \pm 0.34\,\mu$M ($\sim$7$\times$ the uncorrected $0.124\,\mu$M) and HbR $-0.047\rightarrow-0.343\,\mu$M. These magnitudes are physiologically plausible for a finger-tapping task. (Using the two-layer $f_{\text{cortex}}\approx0.10$ instead (ignoring CSF) gives the larger $\kappa_{\text{PV}}\approx9.7$ and $\approx1.3\,\mu$M, an upper bound; the physiological estimate is thus bracketed $\approx0.9$--$1.3\,\mu$M.) This is the payoff of using a \emph{converged}, CSF-augmented $f_{\text{cortex}}$: an under-converged $\approx0.78$ would make $\kappa_{\text{PV}}$ do almost nothing. Consistent with the statistical clarification, $V_{\text{SSR}}$ is reported as a variance diagnostic and is \emph{not} applied.

\subsection{Future Directions}

\begin{enumerate}
\item \textbf{Three-layer Kienle solution} with CSF layer for full anatomical accuracy.
\item \textbf{Hybrid CSF correction}: A particularly promising approach is to compute $f_{\text{cortex}}$ rapidly using the two-layer Hankel transform ($\sim$0.1~s per channel) and then multiply by an SDS-dependent correction factor $\gamma(d_{\text{SDS}}, d_{\text{CSF}})$ from a precomputed lookup table. The adjusted sensitivity $f_{\text{cortex}}^{\text{adj}} = \gamma \times f_{\text{cortex}}^{\text{2L}}$ would retain the speed of the analytical two-layer solution while accounting for the CSF light-guiding effect. Our three-layer analysis shows that $\gamma$ varies systematically: approximately 1.84 at 25~mm, 1.66 at 30~mm, 1.57 at 35~mm, and 1.44 at 40~mm SDS (760~nm).
...
\textit{Solution:} (a)~$0.10\times1.5=0.15$. (b)~$\kappa_{\text{PV}}$ drops from $1/0.10=10$ to $1/0.15\approx6.7$. (c)~Down: a smaller $\kappa_{\text{PV}}$ multiplies the signal by less, so the recovered amplitude decreases (in the paper, the two-layer $\approx1.3~\mu$M upper bound drops to the CSF-augmented $\approx0.9~\mu$M).
...
\[
\texttt{mc\_production.py}\;\rightarrow\;\texttt{fcortex\_production.json}\;\rightarrow\;\texttt{fcortex\_source.py}\;\rightarrow\;\text{both pipelines.}
\]
No analysis script carries a hard-coded sensitivity table or a $760\rightarrow850$ ratio shortcut. The production settings are: anisotropic Henyey--Greenstein scattering $g=0.9$ (with $\mu_s=\mu_s'/(1-g)$), a Fresnel surface, partial-pathlength scoring, $N=2\times10^{6}$ photons per (geometry, wavelength), a converged photon-pathlength cutoff $L_{\max}=1200$~mm and domain depth $z_{\max}=150$~mm (both selected from an observed numerical plateau), and seed~$=1$. Uncertainty on each $f_{\text{cortex}}$ is the spread of the \emph{ratio estimator} across 16 independent photon batches (not a seed-level bootstrap); the file also stores per-SDS DPF and $N_{\text{eff}}$ (an absorption-weight count only), the wavelength-specific CSF ratios $\gamma(\lambda,\text{SDS})$ at 2~mm and 1~mm CSF thickness, and an $L_{\max}$/$z_{\max}$/annulus convergence sweep. The exact per-layer wavelength-specific optical properties used by the MC are stored in the \texttt{fcortex\_production.json} metadata (760~nm: scalp $\mu_a{=}0.0128$, $\mu_s'{=}1.08$; cortex $\mu_a{=}0.0192$, $\mu_s'{=}0.82$; 850~nm: scalp $\mu_a{=}0.0165$, $\mu_s'{=}0.86$; cortex $\mu_a{=}0.0212$, $\mu_s'{=}0.76$~mm$^{-1}$; CSF $\mu_a{=}0.004$, $\mu_s'{=}0.25$/0.24~mm$^{-1}$).
